...
def render_sidebar():
   ...
   # else:
    ...
    # --- CONTROLE DE ANÁLISE EM BACKGROUND ---
    ...
    with col_btn:
        ...
        if not mt5_ok:
            st.sidebar.button("Iniciar Análise", use_container_width=True, help="Conecte ao MT5 primeiro", disabled=True)
        elif not backend_ok:
            st.sidebar.button("Backend Indisponível", use_container_width=True, help="Erro no Sistema Integrado", disabled=True)
        elif sistema_ativo:
            if st.sidebar.button("Parar Análise", use_container_width=True, help="Interrompe o sistema em background"):
                try:
    ...
    # Botão para forçar aplicação imediata dos parâmetros (apenas um botão na sidebar)
    if getattr(st.session_state, 'parametros_alterados', False):
        if st.sidebar.button("🔄 Aplicar Parâmetros Agora", help="Força aplicação imediata dos parâmetros no sistema"):
                try:
                    ...
                    if verificar_parametros_alterados():
                        ...
                        if hasattr(st.session_state.trading_system, 'sistema_integrado') and st.session_state.trading_system.sistema_integrado:
                            ...
                            # Exibe os parâmetros aplicados na área principal, não na sidebar
                            col1, col2 = st.columns(2)
                            with col1:
