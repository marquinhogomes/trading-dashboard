...
class TradingSystemReal:
    @property
    def status_mt5(self):
        """Retorna o status da conexão MT5 para uso no frontend/Streamlit."""
        if not SISTEMA_INTEGRADO_DISPONIVEL:
            return 'indisponivel'
    ...
    def processar_envio_ordens_automatico(self, tabela_linha_operacao01, config):
        """
        Processa envio automático de ordens para os pares da segunda seleção.
        VERSÃO CORRIGIDA COM LOGS DETALHADOS
        """
        try:
            if not self.mt5_connected:
                self.log("❌ MT5 não conectado - impossível enviar ordens")
                return
            
            if tabela_linha_operacao01.empty:
                self.log("⚠️ Nenhum par disponível para envio de ordens")
                return
            
            self.log("🚀 INICIANDO ENVIO AUTOMÁTICO DE ORDENS")
            self.log("="*60)
            ...
            if current_hour < finaliza_ordens:
                for index, linha_selecionada in tabela_linha_operacao01.iterrows():
                    try:
                        ...
                        # Log dos dados da linha para debug
                        ...
                        # Debug: lista todas as colunas disponíveis
                        ...
                        # Validação básica
                        if zscore_hoje is None or pd.isna(zscore_hoje):
                        ...
                        if zscore_hoje is not None and -6.5 < zscore_hoje < -2.0:
                            ...
                            resultado_envio = self._processar_compra_dep_venda_ind_debug(
                                linha_selecionada, depende_atual, independe_atual, 
                                zscore_hoje, beta_hoje, r2_hoje, valor_operacao
                            )
                            if resultado_envio:
                                ordens_enviadas += 1
                            ...
                            resultado_envio = self._processar_venda_dep_compra_ind_debug(
                                linha_selecionada, depende_atual, independe_atual,
                                zscore_hoje, beta_hoje, r2_hoje, valor_operacao
                            )
                            if resultado_envio:
            ...
            self.log(f"✅ ENVIO AUTOMÁTICO CONCLUÍDO: {ordens_enviadas} ordens enviadas")
            self.log("="*60)
            
        except Exception as e:
            self.log(f"❌ Erro geral no envio automático: {str(e)}")
    
    def _processar_compra_dep_venda_ind_debug(self, linha_selecionada, depende_atual, independe_atual, 
                                         zscore_hoje, beta_hoje, r2_hoje, valor_operacao):
        """Versão com debug detalhado da função de compra DEP + venda IND"""
        try:
            self.log(f"🟢 INICIANDO: COMPRA {depende_atual} + VENDA {independe_atual}")
            
            # 1. Obter preços atuais do MT5
            ...
            self.log(f"✅ Cotações obtidas: {depende_atual}={symbol_info_tick_dep.ask:.2f}, {independe_atual}={symbol_info_tick_ind.bid:.2f}")
            
            # 2. Calcular preços de entrada
            self.log(f"💰 Calculando preços de entrada...")
            preco_compra_dep = self._calcular_preco_compra(linha_selecionada, symbol_info_tick_dep)
            preco_venda_ind = self._calcular_preco_venda_indep(linha_selecionada, symbol_info_tick_ind)
            
            if preco_compra_dep is None or preco_venda_ind is None:
                self.log(f"❌ Erro ao calcular preços: compra={preco_compra_dep}, venda={preco_venda_ind}")
                return False
            
            self.log(f"✅ Preços calculados: COMPRA {depende_atual}=R${preco_compra_dep:.2f}, VENDA {independe_atual}=R${preco_venda_ind:.2f}")
            
            # 3. Calcular stops
            self.log(f"🎯 Calculando stops...")
            stop_gain_compra = self._calcular_stop_gain_compra(linha_selecionada, preco_compra_dep)
            stop_loss_compra = self._calcular_stop_loss_compra(linha_selecionada, preco_compra_dep)
            stop_gain_venda_ind = self._calcular_stop_gain_venda_ind(linha_selecionada, preco_venda_ind)
            stop_loss_venda_ind = self._calcular_stop_loss_venda_ind(linha_selecionada, preco_venda_ind)
            
            # 4. Calcular volumes
            self.log(f"📊 Calculando volumes...")
            qtd_dep = self._calcular_volume_operacao(preco_compra_dep, valor_operacao)
            ...
            self.log(f"✅ Volumes: {depende_atual}={qtd_dep}, {independe_atual}={qtd_ind}")
            
            # 5. Validações finais
            self.log(f"✔️ Validando condições...")
            if not self._validar_condicoes_entrada_compra(linha_selecionada, preco_compra_dep):
                self.log(f"❌ Condições de entrada não atendidas")
                return False
            
            # 6. Preparar e enviar ordens
            import time as time_module_local
            magic_id = linha_selecionada.get('ID', int(time_module_local.time()))
            
            self.log(f"📤 ENVIANDO ORDENS - Magic ID: {magic_id}")
            
            # Simula envio (REMOVA ESTA LINHA EM PRODUÇÃO)
            #self.log(f"🧪 SIMULAÇÃO: Ordens seriam enviadas aqui")
            #return True  # REMOVA EM PRODUÇÃO
            
            # DESCOMENTE PARA ENVIO REAL:
            resultado_dep = self._enviar_ordem_mt5(
                symbol=depende_atual,
                volume=qtd_dep,
                order_type=mt5.ORDER_TYPE_BUY_LIMIT,
                price=preco_compra_dep,
                tp=stop_gain_compra,
                sl=stop_loss_compra,
                magic=magic_id,
                comment=f"AutoDep_Z{zscore_hoje:.1f}_R{r2_hoje:.2f}"
            )

            if resultado_dep:
                resultado_ind = self._enviar_ordem_mt5(
                    symbol=independe_atual,
                    volume=qtd_ind,
                    order_type=mt5.ORDER_TYPE_SELL_LIMIT,
                    price=preco_venda_ind,
                    tp=stop_gain_venda_ind,
                    sl=stop_loss_venda_ind,
                    magic=magic_id,
                    comment=f"AutoInd_B{beta_hoje:.2f}"
                )
            ...
            self.log(f"❌ ERRO DETALHADO na compra DEP: {str(e)}")
            self.log(f"❌ TRACEBACK: {traceback.format_exc()[:200]}...")
            return False

    def _processar_venda_dep_compra_ind_debug(self, linha_selecionada, depende_atual, independe_atual,
                                         zscore_hoje, beta_hoje, r2_hoje, valor_operacao):
        """Versão com debug detalhado da função de venda DEP + compra IND"""
        try:
            self.log(f"🔴 INICIANDO: VENDA {depende_atual} + COMPRA {independe_atual}")
            ...
            self.log(f"✅ Preços calculados: VENDA {depende_atual}=R${preco_venda_dep:.2f}, COMPRA {independe_atual}=R${preco_compra_ind:.2f}")
            
            # 3. Calcular stops
            self.log(f"🎯 Calculando stops...")
            stop_gain_venda = self._calcular_stop_gain_venda(linha_selecionada, preco_venda_dep)
            stop_loss_venda = self._calcular_stop_loss_venda(linha_selecionada, preco_venda_dep)
            stop_gain_compra_ind = self._calcular_stop_gain_compra_ind(linha_selecionada, preco_compra_ind)
            stop_loss_compra_ind = self._calcular_stop_loss_compra_ind(linha_selecionada, preco_compra_ind)
            
            # 4. Calcular volumes
            self.log(f"📊 Calculando volumes...")
            qtd_dep = self._calcular_volume_operacao(preco_venda_dep, valor_operacao)
            qtd_ind = round(qtd_dep * abs(beta_hoje), -2)
            ...
            # 5. Validações finais
            self.log(f"✔️ Validando condições...")
            if not self._validar_condicoes_entrada_venda(linha_selecionada, preco_venda_dep):
                self.log(f"❌ Condições de entrada não atendidas")
                return False
            ...
            # ENVIO REAL DAS ORDENS:
            resultado_dep = self._enviar_ordem_mt5(
                symbol=depende_atual,
                volume=qtd_dep,
                order_type=mt5.ORDER_TYPE_SELL_LIMIT,
                price=preco_venda_dep,
                tp=stop_gain_venda,
                sl=stop_loss_venda,
                magic=magic_id,
                comment=f"AutoDep_Z{zscore_hoje:.1f}_R{r2_hoje:.2f}"
            ...
            self.log(f"❌ ERRO DETALHADO na venda DEP: {str(e)}")
            self.log(f"❌ TRACEBACK: {traceback.format_exc()[:200]}...")
            return False

     # Funções auxiliares para cálculos
    def _calcular_preco_compra(self, linha_selecionada, symbol_info_tick):
        """Calcula preço de compra usando spread_compra da linha"""
        try:
            # CORREÇÃO: Verifica múltiplas fontes de preço
            preco_base = None
            
            # 1. Tenta spread_compra da linha (dados da segunda seleção)
            if 'spread_compra' in linha_selecionada and linha_selecionada['spread_compra'] is not None:
                preco_base = float(linha_selecionada['spread_compra'])
                self.log(f"🔧 Preço compra da linha: R$ {preco_base:.2f}")
            
            # 2. Fallback: usa ask do MT5 com desconto para limit order
            else:
                preco_base = symbol_info_tick.ask * 0.998  # 0.2% abaixo do ask para execução
                self.log(f"🔧 Preço compra calculado (ask-0.2%): R$ {preco_base:.2f}")
            
            return round(preco_base, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular preço compra: {str(e)}")
            # Fallback seguro
            return round(symbol_info_tick.ask * 0.998, 2)

    def _calcular_preco_venda(self, linha_selecionada, symbol_info_tick):
        """Calcula preço de venda usando spread_venda da linha"""
        try:
            # CORREÇÃO: Verifica múltiplas fontes de preço
            preco_base = None
            
            # 1. Tenta spread_venda da linha (dados da segunda seleção)
            if 'spread_venda' in linha_selecionada and linha_selecionada['spread_venda'] is not None:
                preco_base = float(linha_selecionada['spread_venda'])
                self.log(f"🔧 Preço venda da linha: R$ {preco_base:.2f}")
            
            # 2. Fallback: usa bid do MT5 com ágio para limit order
            else:
                preco_base = symbol_info_tick.bid * 1.002  # 0.2% acima do bid para execução
                self.log(f"🔧 Preço venda calculado (bid+0.2%): R$ {preco_base:.2f}")
            
            return round(preco_base, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular preço venda: {str(e)}")
            # Fallback seguro
            return round(symbol_info_tick.bid * 1.002, 2)
        
    def _calcular_stop_gain_compra(self, linha_selecionada, preco_entrada):
        """Calcula stop gain para compra"""
        try:
            # CORREÇÃO: Busca dados específicos ou calcula baseado no preço
            if 'spread_compra_gain' in linha_selecionada and linha_selecionada['spread_compra_gain'] is not None:
                tp = float(linha_selecionada['spread_compra_gain'])
                self.log(f"🎯 TP compra da linha: R$ {tp:.2f}")
            else:
                # Calcula TP conservador: 1.5% de ganho
                tp = preco_entrada * 1.015
                self.log(f"🎯 TP compra calculado (1.5%): R$ {tp:.2f}")
            
            return round(tp, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular TP compra: {str(e)}")
            return round(preco_entrada * 1.015, 2)

    def _calcular_stop_loss_compra(self, linha_selecionada, preco_entrada):
        """Calcula stop loss para compra"""
        try:
            # CORREÇÃO: Busca dados específicos ou calcula baseado no preço
            if 'spread_compra_loss' in linha_selecionada and linha_selecionada['spread_compra_loss'] is not None:
                sl = float(linha_selecionada['spread_compra_loss'])
                self.log(f"🛑 SL compra da linha: R$ {sl:.2f}")
            else:
                # Calcula SL conservador: 1% de perda
                sl = preco_entrada * 0.99
                self.log(f"🛑 SL compra calculado (1%): R$ {sl:.2f}")
            
            return round(sl, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular SL compra: {str(e)}")
            return round(preco_entrada * 0.99, 2)

    def _calcular_stop_gain_venda(self, linha_selecionada, preco_entrada):
            """Calcula stop gain para venda"""
            try:
                # CORREÇÃO: Busca dados específicos ou calcula baseado no preço
                if 'spread_venda_gain' in linha_selecionada and linha_selecionada['spread_venda_gain'] is not None:
                    tp = float(linha_selecionada['spread_venda_gain'])
                    self.log(f"🎯 TP venda da linha: R$ {tp:.2f}")
                else:
                    # Calcula TP conservador: 1.5% de ganho (para venda = preço menor)
                    tp = preco_entrada * 0.985
                    self.log(f"🎯 TP venda calculado (1.5%): R$ {tp:.2f}")
                
                return round(tp, 2)
                
            except Exception as e:
                self.log(f"❌ Erro ao calcular TP venda: {str(e)}")
                return round(preco_entrada * 0.985, 2)
        
    def _calcular_stop_loss_venda(self, linha_selecionada, preco_entrada):
        """Calcula stop loss para venda"""
        try:
            # CORREÇÃO: Busca dados específicos ou calcula baseado no preço
            if 'spread_venda_loss' in linha_selecionada and linha_selecionada['spread_venda_loss'] is not None:
                sl = float(linha_selecionada['spread_venda_loss'])
                self.log(f"🛑 SL venda da linha: R$ {sl:.2f}")
            else:
                # Calcula SL conservador: 1% de perda (para venda = preço maior)
                sl = preco_entrada * 1.01
                self.log(f"🛑 SL venda calculado (1%): R$ {sl:.2f}")
            
            return round(sl, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular SL venda: {str(e)}")
            return round(preco_entrada * 1.01, 2)

    def _calcular_preco_compra_indep(self, linha_selecionada, symbol_info_tick):
        """Calcula preço de compra do independente"""
        try:
            # CORREÇÃO: Busca dados específicos do independente
            preco_base = None
            
            # 1. Tenta dados específicos do independente
            if 'indep_spread_compra' in linha_selecionada and linha_selecionada['indep_spread_compra'] is not None:
                preco_base = float(linha_selecionada['indep_spread_compra'])
                self.log(f"🔧 Preço compra indep da linha: R$ {preco_base:.2f}")
            
            # 2. Fallback: usa ask do MT5 com desconto
            else:
                preco_base = symbol_info_tick.ask * 0.998
                self.log(f"🔧 Preço compra indep calculado: R$ {preco_base:.2f}")
            
            return round(preco_base, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular preço compra independente: {str(e)}")
            return round(symbol_info_tick.ask * 0.998, 2)

    def _calcular_preco_venda_indep(self, linha_selecionada, symbol_info_tick):
        """Calcula preço de venda do independente"""
        try:
            # CORREÇÃO: Busca dados específicos do independente
            preco_base = None
            
            # 1. Tenta dados específicos do independente
            if 'indep_spread_venda' in linha_selecionada and linha_selecionada['indep_spread_venda'] is not None:
                preco_base = float(linha_selecionada['indep_spread_venda'])
                self.log(f"🔧 Preço venda indep da linha: R$ {preco_base:.2f}")
            
            # 2. Fallback: usa bid do MT5 com ágio
            else:
                preco_base = symbol_info_tick.bid * 1.002
                self.log(f"🔧 Preço venda indep calculado: R$ {preco_base:.2f}")
            
            return round(preco_base, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular preço venda independente: {str(e)}")
            return round(symbol_info_tick.bid * 1.002, 2)
    ...
    def _calcular_stop_gain_venda_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            # CORREÇÃO: Busca dados específicos do independente
            if 'indep_spread_venda_gain' in linha_selecionada and linha_selecionada['indep_spread_venda_gain'] is not None:
                tp = float(linha_selecionada['indep_spread_venda_gain'])
                self.log(f"🎯 TP venda indep da linha: R$ {tp:.2f}")
            else:
                # Calcula TP para venda do independente: 1.5% abaixo
                tp = preco_entrada * 0.985
                self.log(f"🎯 TP venda indep calculado (1.5%): R$ {tp:.2f}")
            
            return round(tp, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular TP venda independente: {str(e)}")
            return round(preco_entrada * 0.985, 2)
    ...
    def _calcular_stop_loss_venda_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            # CORREÇÃO: Busca dados específicos do independente
            if 'indep_spread_venda_loss' in linha_selecionada and linha_selecionada['indep_spread_venda_loss'] is not None:
                sl = float(linha_selecionada['indep_spread_venda_loss'])
                self.log(f"🛑 SL venda indep da linha: R$ {sl:.2f}")
            else:
                # Calcula SL para venda do independente: 1% acima
                sl = preco_entrada * 1.01
                self.log(f"🛑 SL venda indep calculado (1%): R$ {sl:.2f}")
            
            return round(sl, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular SL venda independente: {str(e)}")
            return round(preco_entrada * 1.01, 2)
    ...
    def _calcular_stop_gain_compra_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            # CORREÇÃO: Busca dados específicos do independente
            if 'indep_spread_compra_gain' in linha_selecionada and linha_selecionada['indep_spread_compra_gain'] is not None:
                tp = float(linha_selecionada['indep_spread_compra_gain'])
                self.log(f"🎯 TP compra indep da linha: R$ {tp:.2f}")
            else:
                # Calcula TP para compra do independente: 1.5% acima
                tp = preco_entrada * 1.015
                self.log(f"🎯 TP compra indep calculado (1.5%): R$ {tp:.2f}")
            
            return round(tp, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular TP compra independente: {str(e)}")
            return round(preco_entrada * 1.015, 2)
    ...
    def _calcular_stop_loss_compra_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            # CORREÇÃO: Busca dados específicos do independente
            if 'indep_spread_compra_loss' in linha_selecionada and linha_selecionada['indep_spread_compra_loss'] is not None:
                sl = float(linha_selecionada['indep_spread_compra_loss'])
                self.log(f"🛑 SL compra indep da linha: R$ {sl:.2f}")
            else:
                # Calcula SL para compra do independente: 1% abaixo
                sl = preco_entrada * 0.99
                self.log(f"🛑 SL compra indep calculado (1%): R$ {sl:.2f}")
            
            return round(sl, 2)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular SL compra independente: {str(e)}")
            return round(preco_entrada * 0.99, 2)
    ...
    def _calcular_volume_operacao(self, preco_entrada, valor_operacao):
        """Calcula volume baseado no valor da operação"""
        try:
            if preco_entrada <= 0 or valor_operacao <= 0:
                self.log(f"❌ Valores inválidos: preco={preco_entrada}, valor={valor_operacao}")
                return 100.0
            
            volume_bruto = valor_operacao / preco_entrada
            # Arredonda para múltiplos de 100 (lote mínimo típico)
            volume_final = max(100, round(volume_bruto / 100) * 100)
            
            self.log(f"📊 Volume calculado: R$ {valor_operacao} / R$ {preco_entrada:.2f} = {volume_final}")
            return float(volume_final)
            
        except Exception as e:
            self.log(f"❌ Erro ao calcular volume: {str(e)}")
            return 100.0

    def _validar_condicoes_entrada_compra(self, linha_selecionada, preco_compra):
        """Valida condições para entrada em compra - SIMPLIFICADO"""
        try:
            # CORREÇÃO: Validação simplificada baseada em dados disponíveis
            preco_atual = linha_selecionada.get('preco_atual', 0)
            
            if preco_atual > 0 and preco_compra > 0:
                # Compra deve ser abaixo do preço atual (limit order)
                validacao = preco_compra <= preco_atual
                self.log(f"✅ Validação compra: preço_compra={preco_compra:.2f} <= preco_atual={preco_atual:.2f} = {validacao}")
                return validacao
            
            # Se não tem dados, permite a operação
            self.log(f"⚠️ Validação compra: dados insuficientes, permitindo operação")
            return True
            
        except Exception as e:
            self.log(f"❌ Erro na validação compra: {str(e)}")
            return True  # Default para permitir operação

    def _validar_condicoes_entrada_venda(self, linha_selecionada, preco_venda):
        """Valida condições para entrada em venda - SIMPLIFICADO"""
        try:
            # CORREÇÃO: Validação simplificada baseada em dados disponíveis
            preco_atual = linha_selecionada.get('preco_atual', 0)
            
            if preco_atual > 0 and preco_venda > 0:
                # Venda deve ser acima do preço atual (limit order)
                validacao = preco_venda >= preco_atual
                self.log(f"✅ Validação venda: preco_venda={preco_venda:.2f} >= preco_atual={preco_atual:.2f} = {validacao}")
                return validacao
            
            # Se não tem dados, permite a operação
            self.log(f"⚠️ Validação venda: dados insuficientes, permitindo operação")
            return True
            
        except Exception as e:
            self.log(f"❌ Erro na validação venda: {str(e)}")
            return True  # Default para permitir operação
        
    def _verificar_operacao_aberta(self, lista_ativos):
        """Verifica se já existe operação aberta para os ativos"""
        try:
    ...
    def executar_analise_real(self, config: Dict):
        ...
        try:
            ...
            # ========================================================================
            self.log("🎯 Executando SEGUNDA SELEÇÃO (refinamento SEM recálculo de indicadores)...")
            ...
            for linha in tabela_linha_operacao.itertuples():
                ...
                try:
                    # ✅ CORREÇÃO CRÍTICA: Extrai valores ORIGINAIS da primeira seleção
                    ...
                    if resultado and len(resultado) >= 30:
                        ...
                        if zscore_original is not None and abs(zscore_original) > 1.5:
                            # 🔧 CORREÇÃO: Log de confirmação COM OS MESMOS VALORES
                            self.log(f"✅ Par {dependente_atual01}x{independente_atual01} (segunda seleção): PRESERVANDO zscore={zscore_original:.3f}, r2={r2_original:.3f}")
                            
                            # ✅ VALIDAÇÃO ADICIONAL: Confirma que os valores são os mesmos
                            self.log(f"🔍 VALIDAÇÃO: {dependente_atual01}x{independente_atual01} mantém Z={zscore_original:.3f}, R²={r2_original:.3f} da 1ª seleção")
            ...
            if linha_operacao01:
                ...
                self.processar_envio_ordens_automatico(tabela_linha_operacao01, config)
            ...
            try:
                if self.modo_otimizado:
                    ...
                    if hasattr(self, 'sinais_ativos') and self.sinais_ativos:
                        dados_para_sincronizar['sinais_ativos'] = self.sinais_ativos.copy()
                    ...
                    if hasattr(self, 'equity_historico') and self.equity_historico:
                        dados_para_sincronizar['equity_historico'] = self.equity_historico.copy()
                    
                    if hasattr(self, 'posicoes_abertas') and self.posicoes_abertas:
                        dados_para_sincronizar['posicoes_abertas'] = self.posicoes_abertas.copy()
