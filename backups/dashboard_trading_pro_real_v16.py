...
def render_sidebar():
   ...
   # else:
    ...
    valor_operacao = st.sidebar.number_input("Valor máximo por operação (DEP)", min_value=1000, max_value=100000, value=10000, step=500)
    valor_operacao_ind = st.sidebar.number_input("Valor máximo por operação (IND)", min_value=1000, max_value=100000, value=10000, step=500)
    limite_lucro = st.sidebar.number_input("Limite de lucro por operação", min_value=10, max_value=1000, value=120, step=10)
    limite_prejuizo = st.sidebar.number_input("Limite de prejuízo por operação", min_value=10, max_value=1000, value=120, step=10)

    # INTEGRAÇÃO: Atualiza variáveis globais do módulo de cálculo com os valores do dashboard
    import sys
    if 'calculo_entradas_v55' in sys.modules:
        calc_mod = sys.modules['calculo_entradas_v55']
    else:
