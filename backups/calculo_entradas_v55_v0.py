...
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # Desabilita as operações customizadas do oneDNN
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'  # Suprime avisos INFO e WARNING do TensorFlow
try:
...
GARCH_FALLBACK_MULTIPLIER = 0.02  # Multiplier para fallback quando GARCH falha (2%)
...
periodo = [70, 100, 120, 140, 160, 180, 200, 220, 240, 250]

# ─────────── Configuração de Horários Otimizada ───────────
inicia_app = 8                    # App inicia às 9h (pré-abertura)
finaliza_app = 24                 # App finaliza às 18h (pós-fechamento)
inicia_pregao = 10                # Pregão oficial: 10h
finaliza_pregao = 24              # Pregão oficial: 17h (mantido)
...
finaliza_ordens = 24              # Para de enviar NOVAS ordens às 15h (mantido)
ajusta_ordens = 24                # Ajustes às 15h (mantido)
ajusta_ordens_minuto = 10         # Ajustes às 15:10h (mantido)

# Horários específicos para diferentes tipos de ajuste
horario_ajuste_stops = 24         # 15h - Ajustar stops de posições abertas
horario_remove_pendentes = 24     # 15h - Remover ordens pendentes (15:20h)
horario_fechamento_total = 24     # 16h - Fechamento forçado (16:01h)

# NOVO: Janelas de tempo para diferentes atividades
JANELA_ANALISE_POSICOES = (8, 24)      # 10h-17h: Monitorar posições
JANELA_NOVAS_OPERACOES = (8, 24)       # 10h-16h: Aceitar novas operações
JANELA_AJUSTES_DINAMICOS = (8, 24)     # 10h-17h: Ajustes dinâmicos
JANELA_BREAK_EVEN = (8, 24)            # 10h-17h: Break-even automático
...
desvio_gain_compra = 1.012      # 1.2-1.012, 1.1-1.011, 1.0-1.010, 0.9-1.009, 0.8-1.008, 0.7-1.007, 0.6-1.006
desvio_loss_compra = 0.988      # 1.2-0.988, 1.1-0.989, 1.0-0.990, 0.9-0.991, 0.8-0.992, 0.7-0.993, 0.6-0.994
desvio_gain_venda = 0.988       # 1.2-0.988, 1.1-0.989, 1.0-0.990, 0.9-0.991, 0.8-0.992, 0.7-0.993, 0.6-0.994 
desvio_loss_venda = 1.012       # 1.2-1.012, 1.1-1.011, 1.0-1.010, 0.9-1.009, 0.8-1.008, 0.7-1.007, 0.6-1.006
desvio_gain_compra_ind = 1.03      # 1.2-1.012, 1.1-1.011, 1.0-1.010, 0.9-1.009, 0.8-1.008, 0.7-1.007, 0.6-1.006
desvio_loss_compra_ind = 0.97      # 1.2-0.988, 1.1-0.989, 1.0-0.990, 0.9-0.991, 0.8-0.992, 0.7-0.993, 0.6-0.994
desvio_gain_venda_ind = 0.97       # 1.2-0.988, 1.1-0.989, 1.0-0.990, 0.9-0.991, 0.8-0.992, 0.7-0.993, 0.6-0.994 
desvio_loss_venda_ind = 1.03       # 1.2-1.012, 1.1-1.011, 1.0-1.010, 0.9-1.009, 0.8-1.008, 0.7-1.007, 0.6-1.006
...
def calcular_residuo_zscore_timeframe(dep, ind, ibov, win, periodo, dados_preprocessados, 
                                      USE_SPREAD_FORECAST=True, zscore_threshold=2.0, verbose=False,
                                      enable_zscore_filter=True, enable_r2_filter=True, enable_beta_filter=True,
                                      enable_cointegration_filter=True,
                                      zscore_min_threshold=-2.0, zscore_max_threshold=2.0,
                                      r2_min_threshold=0.50, beta_max_threshold=1.5):
    """
...
def encontrar_linha_monitorada(tabela_zscore_mesmo_segmento, linha_operacao, dados_preprocessados, filter_params=None, enable_cointegration_filter=True):
    ...
    zscore_extremo = ((tabela_zscore_mesmo_segmento['Z-Score'] > 2.0) | (tabela_zscore_mesmo_segmento['Z-Score'] < -2.0))
    ...
    for _, linha in selecao.iterrows():
        ...
        id_val = linha['ID'] if 'ID' in linha.index else None
        ...
        filtros_passaram['beta'] = float(beta) < filter_params.get('beta_max', 1.5) if not pd.isna(beta) else False
...
def encontrar_linha_monitorada01(tabela_zscore_dependente_atual01, linha_operacao01=None):
    ...
    for col in ['Z-Score','beta','r2','correlacao_ibov','beta_rotation','beta_rotation_mean', 'preco_max_atual', 'preco_min_atual', 'previsao_maximo', 'previsao_minimo', 'preco_atual', 'previsao_fechamento', 'preco_atual_indep', 'previsao_fechamento_ind', 'forecast', 'residuo'  ]:
    ...
    # suas máscaras originais
    #cond_z          = (tabela['Z-Score'].abs() >= 2.0)
    #cond_r2         = tabela['r2'] >= 0.5
    cond_preco_max  = (tabela['Z-Score'] >= 2.0) & (tabela['beta_rotation'] > tabela['beta_rotation_mean'])
    cond_preco_min  = (tabela['Z-Score'] <= -2.0) & (tabela['beta_rotation'] < tabela['beta_rotation_mean'])

    # combina tudo num único filtro
    mask = (cond_preco_max | cond_preco_min) #cond_z & cond_r2 
    ...
    for idx, row in tabela_filtrada.iterrows():
        dep = row['Dependente']
        zscore = row['Z-Score']
        beta_rot = row['beta_rotation']
        beta_mean = row['beta_rotation_mean']
        
        if zscore >= 2.0 and beta_rot > beta_mean:
            print(f"[DEBUG] {dep}: VENDA (Z={zscore:.2f}, β_rot={beta_rot:.3f} > β_mean={beta_mean:.3f})")
        elif zscore <= -2.0 and beta_rot < beta_mean:
...
def analisar_diferencas_previsoes_internas(dados_historicos_previsoes, salvar_arquivo=True):
    ...
    # NOVO: Salva/acumula o histórico completo de dados analisados
    import os
    arquivo_historico = "historico_analises.xlsx"
    if os.path.exists(arquivo_historico):
        df_historico = pd.read_excel(arquivo_historico)
    else:
        df_historico = pd.DataFrame()
    df_historico = pd.concat([df_historico, dados_historicos_previsoes], ignore_index=True)
    # (Opcional) Remove duplicatas por colunas-chave
    # df_historico = df_historico.drop_duplicates(subset=['ativo', 'tipo_ativo', 'periodo', 'timeframe', 'data'])
    df_historico.to_excel(arquivo_historico, index=False)
    
    # Use o histórico completo para análise
    dados_para_analisar = df_historico

    resultados = []
    grupos = dados_para_analisar.groupby(['ativo', 'tipo_ativo', 'periodo', 'timeframe'])
    ...
    # Salva/acumula todos os resultados das análises em um histórico incremental
    if not df_analise.empty:
        arquivo_resultados = "historico_resultados_diferencas.xlsx"
        if os.path.exists(arquivo_resultados):
            df_resultados_antigos = pd.read_excel(arquivo_resultados)
        else:
            df_resultados_antigos = pd.DataFrame()
        # Concatena e remove duplicatas por colunas-chave (ajuste conforme necessário)
        df_resultados_todos = pd.concat([df_resultados_antigos, df_analise], ignore_index=True)
        df_resultados_todos = df_resultados_todos.drop_duplicates(subset=['ativo', 'tipo_ativo', 'periodo', 'timeframe', 'data_fim'], keep='last')
        df_resultados_todos.to_excel(arquivo_resultados, index=False)
        print(f"[INFO] Histórico de diferenças das previsões atualizado em: {arquivo_resultados}")
        # (Opcional) Também salva o resultado atual em arquivo separado, como antes
...
def analisar_acuracia_previsoes_vs_real(dados_historicos_completos, salvar_arquivo=True):
    ...
    # NOVO: Salva/acumula o histórico completo de dados analisados
    import os
    arquivo_historico = "historico_analises_completas.xlsx"
    if os.path.exists(arquivo_historico):
        df_historico = pd.read_excel(arquivo_historico)
    else:
        df_historico = pd.DataFrame()
    df_historico = pd.concat([df_historico, dados_historicos_completos], ignore_index=True)
    # (Opcional) Remove duplicatas por colunas-chave
    # df_historico = df_historico.drop_duplicates(subset=['ativo', 'tipo_ativo', 'periodo', 'timeframe', 'data'])
    df_historico.to_excel(arquivo_historico, index=False)
    ...
    if salvar_arquivo and not df_acuracia.empty:
        arquivo_resultados = "historico_resultados_analises.xlsx"
        if os.path.exists(arquivo_resultados):
            df_resultados_antigos = pd.read_excel(arquivo_resultados)
        else:
            df_resultados_antigos = pd.DataFrame()
        # Concatena e remove duplicatas por colunas-chave (ajuste conforme necessário)
        df_resultados_todos = pd.concat([df_resultados_antigos, df_acuracia], ignore_index=True)
        # Exemplo: evitar duplicatas por ativo, tipo_ativo, periodo, timeframe, data_fim
        df_resultados_todos = df_resultados_todos.drop_duplicates(subset=['ativo', 'tipo_ativo', 'periodo', 'timeframe', 'data_fim'], keep='last')
        df_resultados_todos.to_excel(arquivo_resultados, index=False)
        print(f"[INFO] Histórico de resultados das análises atualizado em: {arquivo_resultados}")
        # (Opcional) Também salva o resultado atual em arquivo separado, como antes
...
def main(loop=True, timeframe_atual=None, filter_params=None):
    ...
    #global dados_historicos, ultima_atualizacao
       # Inicializa filter_params com valores padrão
   
    filter_params = {
        'r2_min': 0.5,              # Reduzido de 0.4 para 0.1 (10%)
        'beta_max': 1.5,            # Aumentado de 1.0 para 2.0
        #'residuo_std_min': 0.0,     # Mantido em 0.0 
        #'correlacao_max': 0.9,      # Correlação máxima permitida
        'coef_var_max': 5000.0,      # Mantido
        'adf_p_value_max': 0.05,    # Mantido
        'use_coint_test': True,
        'use_adf_critical': False,
        'enable_cointegration_filter': True,  # Habilita/desabilita filtro de cointegração global
        #'confidence_level': '95%',   # Nível de confiança para testes
        #'coint_index': 1,           # Índice de cointegração para 95%
        #'adf_critical_key': '5%'     # Chave crítica ADF para 95%
    }    
            ...
            if inicia_pregao <= current_hour < finaliza_pregao:
                ...
                if linha_operacao01:
                    ...
                    for linha_dict in linha_operacao01:
                        ...
                        try:
                            # Obtém o preço atual do dependente dos dados já calculados
                            preco_atual = linha_dict.get('preco_atual', 0.0)
                            
                            # Usa os spreads já calculados e disponíveis na linha_dict
                            spread_compra = linha_dict.get('spread_compra', preco_atual)
                            spread_venda = linha_dict.get('spread_venda', preco_atual)
                            
                            # Determina o preço de entrada baseado no Z-Score usando spreads já calculados
                            if zscore <= -2.0:
                                # Z-Score <= -2: comprar dependente
                                preco_entrada = spread_compra
                            elif zscore >= 2.0:
                                # Z-Score >= 2: vender dependente  
                                preco_entrada = spread_venda
                            else:
                                # Z-Score neutro: usar preço atual
                                preco_entrada = preco_atual
                            
                            # Calcula o percentual de diferença entre preço de entrada e preço atual
                            if preco_atual > 0:
                                perc_diferenca = abs((preco_entrada - preco_atual) / preco_atual * 100)
                            else:
                                perc_diferenca = 999.0  # Prioridade baixa se preço atual inválido
                            
                            # Adiciona informações de priorização à linha
                            ...
                            #print(f"[DEBUG] {dep}: zscore={zscore:.2f}, entrada={preco_entrada:.2f}, atual={preco_atual:.2f}, diff={perc_diferenca:.3f}%")
                            
                        except Exception as e:
                            print(f"[ERRO] Não foi possível calcular prioridade para {dep}: {e}")
                            # Adiciona com prioridade baixa (valor alto)
                            linha_com_prioridade = linha_dict.copy()
                ...
                if current_hour < finaliza_ordens:
                    for linha_selecionada in linha_operacao01:
                       ...
                       # print(f">>> Iniciando tentativa de abertura de operação para DEPENDENTE ({depende_atual})")
                        ...
                        # Verificar se zscore_hoje não é None antes de usar em comparações
                        if zscore_hoje is not None and 2.0 < zscore_hoje < 6.5:
