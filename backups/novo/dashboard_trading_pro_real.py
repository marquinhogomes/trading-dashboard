...
def render_trade_history():
            ...
            if trades_reais and len(trades_reais) > 0:
                ...
                if not df_display.empty:
                    ...
                    st.dataframe(
                      ...
                      # Calcula e exibe estatísticas reais
    ...
    st.info("🔧 **Conecte ao MT5 para visualizar histórico real de trades**")
...
def render_segunda_selecao():
    ...
    if df_segunda is not None and not df_segunda.empty:
        ...
        if posicoes_segunda:
            df_display = pd.DataFrame(posicoes_segunda)
            ...
            with filter_col1:
                tipos_unicos = ['Todos'] + sorted(df_display['Tipo'].unique().tolist())
                tipo_filter = st.selectbox("Tipo de Posição", tipos_unicos)
            
            with filter_col2:
                setores_unicos = ['Todos'] + sorted(df_display['Setor'].unique().tolist())
                setor_filter = st.selectbox("Setor", setores_unicos)
            
            with filter_col3:
                pl_filter = st.selectbox("P&L", ["Todos", "Positivo", "Negativo"])
            
            with filter_col4:
                show_advanced = st.checkbox("Mostrar Métricas Avançadas", True)
            ...
            if pl_filter == "Positivo":
                df_filtered = df_filtered[df_filtered['P&L (R$)'].str.contains(r'\+')]
            elif pl_filter == "Negativo":
                df_filtered = df_filtered[df_filtered['P&L (R$)'].str.contains(r'-')]
            
            #st.markdown(f"#### 📊 Posições da Segunda Seleção: {len(df_filtered)} de {len(df_display)}")
            
            # Seleciona colunas para exibição
            cols_principais = ['Par', 'Tipo', 'Volume', 'Preço Abertura', 'Preço Atual', 
                             'P&L (R$)', 'P&L (%)', 'Stop Loss', 'Take Profit', 'Tempo Aberto', 'Setor']
            
            if show_advanced:
                cols_principais.extend(['Z-Score', 'R²', 'Beta_Rot', 'Confiança'])
            
            # Aplica cores condicionais (formato profissional)
            def color_pl(val):
                if '+' in str(val):
                    return 'color: green; font-weight: bold'
                elif '-' in str(val):
            ...
            def color_tipo(val):
                if val == 'LONG':
                    ...
                    return 'background-color: rgba(255, 0, 0, 0.1)'  # Vermelho claro
                return 'background-color: rgba(128, 128, 128, 0.1)'  # Cinza claro
            
            # Exibe tabela principal no formato da imagem
            st.dataframe(
                df_filtered[cols_principais].style.applymap(color_tipo, subset=['Tipo'])
                                                  .applymap(color_pl, subset=['P&L (R$)', 'P&L (%)']),
                use_container_width=True,
                hide_index=True,
                height=400
