...
def encontrar_linha_monitorada01(tabela_zscore_dependente_atual01, linha_operacao01=None):
    ...
    # Exemplo: filtro de r2 pode ser incluído se necessário
    # cond_r2 = tabela['r2'] >= r2_min_dyn
    # mask = (cond_preco_max | cond_preco_min) & cond_r2
    ...
    for idx, row in tabela_filtrada.iterrows():
        ...
        if zscore >= zscore_min and beta_rot > beta_mean:
            print(f"[DEBUG] {dep}: VENDA (Z={zscore:.2f}, β_rot={beta_rot:.3f} > β_mean={beta_mean:.3f})")
        elif zscore <= -zscore_min and beta_rot < beta_mean:
