...
class SistemaIntegrado:
    ...
    def __init__(self):
        ...
        self.prefixo = calc_mod.prefixo
        self.limite_operacoes = calc_mod.limite_operacoes
        self.indep_limite_operacoes = calc_mod.indep_limite_operacoes
        self.valor_operacao = calc_mod.valor_operacao
        self.valor_operacao_ind = calc_mod.valor_operacao_ind
        self.limite_lucro = calc_mod.limite_lucro
        self.limite_prejuizo = calc_mod.limite_prejuizo
        self.pvalor = calc_mod.pvalor
        self.apetite_perc_media = calc_mod.apetite_perc_media
        self.finaliza_ordens = calc_mod.finaliza_ordens
        self.ajusta_ordens = calc_mod.ajusta_ordens
        self.ajusta_ordens_minuto = calc_mod.ajusta_ordens_minuto
        self.horario_ajuste_stops = calc_mod.horario_ajuste_stops
        self.horario_remove_pendentes = calc_mod.horario_remove_pendentes
        self.horario_fechamento_total = calc_mod.horario_fechamento_total
        self.JANELA_ANALISE_POSICOES = calc_mod.JANELA_ANALISE_POSICOES
        self.JANELA_NOVAS_OPERACOES = calc_mod.JANELA_NOVAS_OPERACOES
        self.JANELA_AJUSTES_DINAMICOS = calc_mod.JANELA_AJUSTES_DINAMICOS
        self.JANELA_BREAK_EVEN = calc_mod.JANELA_BREAK_EVEN
        self.desvio_gain_compra = calc_mod.desvio_gain_compra
        self.desvio_loss_compra = calc_mod.desvio_loss_compra
        self.desvio_gain_venda = calc_mod.desvio_gain_venda
        self.desvio_loss_venda = calc_mod.desvio_loss_venda
        self.desvio_gain_compra_ind = calc_mod.desvio_gain_compra_ind
        self.desvio_loss_compra_ind = calc_mod.desvio_loss_compra_ind
        self.desvio_gain_venda_ind = calc_mod.desvio_gain_venda_ind
        self.desvio_loss_venda_ind = calc_mod.desvio_loss_venda_ind
        ...
        # Configurações horário (sempre usar as variáveis globais do módulo de cálculo)
        self.JANELA_BREAK_EVEN = calc_mod.JANELA_BREAK_EVEN
        self.horario_ajuste_stops = calc_mod.horario_ajuste_stops
        self.ajusta_ordens_minuto = calc_mod.ajusta_ordens_minuto
        self.horario_remove_pendentes = calc_mod.horario_remove_pendentes
    ...
    def executar_sistema_original(self):
        ...
        try:
            ...
            if 'tabela_linha_operacao' in globals():
                self.tabela_linha_operacao = globals()['tabela_linha_operacao']
            else:
                self.tabela_linha_operacao = None
            if 'tabela_linha_operacao01' in globals():
                self.tabela_linha_operacao01 = globals()['tabela_linha_operacao01']
            else:
    ...
    def thread_analise_e_envio_ordens(self):
        ...
        while self.running:
            try:
                ...
                # Utiliza variáveis globais centralizadas para horário de operação
                if self.horario_ajuste_stops <= current_hour < self.horario_fechamento_total:
                    self.executar_analise_e_envio_ordens()
                # Aguarda próximo ciclo (usa ajuste centralizado)
                for i in range(self.ajusta_ordens_minuto * 60):
                    if not self.running:
                        break
                    time.sleep(1)
            except Exception as e:
    ...
    def executar_analise_e_envio_ordens(self, tabela_linha_operacao01=None, config=None):
        ...
        try:
            ...
            if tabela_linha_operacao01 is None or config is None:
                current_hour = datetime.now().hour
                finaliza_ordens = self.horario_fechamento_total
            ...
            if tabela_linha_operacao01 is not None and config is not None:
                ...
                current_hour = datetime.now().hour
                finaliza_ordens = self.horario_fechamento_total
                limite_operacoes = config.get('max_posicoes', self.limite_operacoes)
                valor_operacao = config.get('valor_operacao', self.valor_operacao)
                prefixo = self.prefixo
                ordens_enviadas = 0
                max_ordens_por_ciclo = self.limite_operacoes
    ...
    # ==================================================================================
    # MÉTODOS WRAPPER PARA INTERFACE DO DASHBOARD
    # ==================================================================================
    ...
    def fechar_posicao(self, ticket):
        ...
        try:
            ...
            if not posicoes:
                self.log(f"❌ Posição com ticket {ticket} não encontrada")
                return False
                
            posicao = posicoes[0]
            self.fechar_posicao_especifica(posicao)
            return True
    ...
    def cancelar_ordem(self, ticket):
        """Cancela uma ordem pendente específica pelo ticket"""
        import MetaTrader5 as mt5
        
        try:
            # Busca a ordem pelo ticket
            ordens = mt5.orders_get(ticket=ticket)
            if not ordens:
                self.log(f"❌ Ordem com ticket {ticket} não encontrada")
                return False
    ...
    def iniciar_threads_apenas(self):
        """Inicia apenas as threads sem entrar no loop de monitoramento (para uso com dashboard)"""
        self.log("🎯 INICIANDO THREADS DO SISTEMA INTEGRADO (Modo Dashboard)")
        self.log("=" * 80)
        self.log("Iniciando threads:")
        self.log("✅ Sistema Trading")
        self.log("✅ Monitoramento Geral")  
        self.log("✅ Monitoramento de Posições")
        self.log("✅ Break-even Contínuo")
        self.log("✅ Ajustes Programados")
        self.log("✅ Análise e Envio de Ordens")
        self.log("=" * 80)
        ...
        # Thread: Ajustes programados
        thread_ajustes = threading.Thread(target=self.thread_ajustes_programados, name="AjustesProgramados")
        # Thread: Análise e envio de ordens
        thread_ordens = threading.Thread(target=self.thread_analise_e_envio_ordens, name="AnaliseEnvioOrdens")
        ...
        self.log("✅ Todas as threads iniciadas - Sistema operacional em modo Dashboard!")
        self.log("🔍 Thread de monitoramento de posições: A cada 30 segundos")
        self.log("📈 Thread de break-even contínuo: A cada 10 segundos durante pregão")
        self.log("⏰ Thread de ajustes programados: Horários específicos (15:10h, 15:20h, 16:01h)")
        self.log("📊 Thread de análise e envio de ordens: A cada 5 minutos durante pregão")
        
        # RETORNA IMEDIATAMENTE (sem loop bloqueante)
        return True
