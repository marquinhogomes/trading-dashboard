...
def initialize_session_state():
    ...
    if HAS_REAL_SYSTEM and real_trading_system:
        st.session_state.real_system = real_trading_system
        st.session_state.real_state = real_state
    else:
        st.session_state.real_system = None
        st.session_state.real_state = None
    ...
    if 'config' not in st.session_state:
        if HAS_REAL_SYSTEM and REAL_CONFIG:
...
def render_connection_status():
    ...
    if HAS_REAL_SYSTEM and 'real_trading_state' in st.session_state:
        ...
        with col2:
            ...
            has_original_code = real_status.get('has_original_code', False)
        ...
        if system.mt5_connected:
          ...
          # ETAPA 1: Métricas do sistema real
        ...
        if st.button("🔍 Executar Análise"):
            with st.spinner("Analisando pares..."):
                if use_real_system:
                    ...
                    real_results = get_real_analysis_data()
        ...
        if st.button("🔄 Reanalizar Pares"):
            if use_real_system:
                with st.spinner("Executando nova análise..."):
                    ...
                    if pairs:
                        new_results = get_real_analysis_data()
...
def render_real_system_logs():
    ...
    # Obter logs
    logs = real_state.logs[-limit:] if real_state.logs else []
...
def render_dashboard():
    ...
    if use_real_data:
        ...
        with col1:
            ...
            if 'best_trades' in real_status and real_status.get('best_trades'):
                best_trades_data = real_status.get('best_trades', [])
        ...
        with col2:
            ...
            if 'worst_trades' in real_status and real_status.get('worst_trades'):
                worst_trades_data = real_status.get('worst_trades', [])
...
def render_trading_control_panel():
    ...
    with col3:
        if st.button("🔄 Executar Análise Manual", key="manual_analysis"):
            with st.spinner("Executando análise..."):
                pairs = st.session_state.config.get('pairs', ['EURUSD', 'GBPUSD'])
                results = get_real_analysis_data()
            ...
            with st.spinner("Executando a função principal do sistema original..."):
                try:
                    ...
                    def run_original_main():
                        try:
                            # Executar a função main do código original
                            ORIGINAL_FUNCTIONS['main']()
                            real_state.log("Função principal executada com sucesso", "INFO")
                        except Exception as e:
                            real_state.log(f"Erro na execução da função principal: {e}", "ERROR")
    ...
    recent_logs = real_state.logs[-10:] if real_state.logs else []
    if recent_logs:
