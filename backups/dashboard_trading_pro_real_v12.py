...
# ✅ AUTO-REFRESH: Atualização automática da interface configurável
# Inicializa configurações padrão apenas uma vez
...
# Usa st_autorefresh para garantir atualização automática
...
elif st.session_state.auto_refresh_enabled and not st_autorefresh:
    st.warning("Pacote 'streamlit-autorefresh' não instalado. O auto-refresh pode não funcionar corretamente.")

# 🔄 AUTO-REFRESH ATIVO: Executa atualização automática se habilitado
if st.session_state.auto_refresh_enabled:
    current_time = datetime.now()
    time_since_refresh = (current_time - st.session_state.last_auto_refresh).total_seconds()
    
    # Debug: mostra tempo decorrido
    print(f"[DEBUG] Auto-refresh: {time_since_refresh:.1f}s desde última atualização (limite: {st.session_state.auto_refresh_interval}s)")
    ...
    if time_since_refresh >= st.session_state.auto_refresh_interval:
        ...
        if hasattr(st.session_state, 'trading_system') and st.session_state.trading_system:
            st.session_state.trading_system.log(f"🔄 Auto-refresh executado após {time_since_refresh:.1f}s")
        print(f"[AUTO-REFRESH] Página recarregada após {time_since_refresh:.1f}s")
        st.rerun()
    
    # 🆘 FALLBACK: Se Streamlit não executar auto-refresh em 5s extras, força execução
    elif time_since_refresh >= (st.session_state.auto_refresh_interval + 5):
        print(f"[FALLBACK] Forçando auto-refresh após {time_since_refresh:.1f}s")
        st.session_state.last_auto_refresh = current_time
        st.rerun()
...
class TradingSystemReal:
    ...
    def _criar_segunda_selecao_fallback(self):
        ...
        try:
            ...
            # 1. Z-Score absoluto >= 2.0
            # 2. R² >= 0.6
            # 3. Prioriza pares com melhor potencial
            mascara_segunda = (
            ...
            self.log(f"📊 Critérios: Z-Score >= 2.0, R² >= 0.6, ordenados por qualidade")
        ...
        if not posicoes:
            ...
            if posicoes_formatted:
                ...
                for i, pos_data in enumerate(posicoes_formatted):
                    with st.container():
                        # Obtém dados da posição original para ações
                        pos_original = posicoes[i]
                        ticket = pos_original.get('ticket')
                        symbol = pos_original.get('symbol', 'N/A')
                        pl_value = pos_original.get('profit', 0)
                        
                        # Layout em colunas: informações + botão de ação
                        ...
                        with col_info:
                            ...
                            st.markdown(f"""
                            <div style="padding: 8px; border-radius: 8px; border: 1px solid #ddd; margin-bottom: 8px;">
                        ...
                        with col_action:
                            ...
                            if st.button(f"❌ Fechar", key=f"close_pos_{ticket}_{i}", type="secondary", use_container_width=True):
                                if sistema.fechar_posicao(ticket):
        ...
        if not ordens_pendentes:
            ...
            if ordens_formatted:
                ...
                for i, ordem_data in enumerate(ordens_formatted):
                    with st.container():
                        # Obtém dados da ordem original para ações
                        ordem_original = ordens_pendentes[i]
                        ticket = ordem_original.get('ticket')
                        ...
                        with col_action:
                            ...
                            if st.button(f"🚫 Cancelar", key=f"cancel_order_{ticket}_{i}", type="secondary", use_container_width=True):
