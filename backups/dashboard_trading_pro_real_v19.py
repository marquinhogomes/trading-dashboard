...
class TradingSystemReal:
    ...
    def _calcular_stop_gain_compra(self, linha_selecionada, preco_entrada):
        ...
        try:
            ...
            stop_gain_pct = config.get('stop_gain_compra_pct', 1.015)
    ...
    def _calcular_stop_loss_compra(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_loss_pct = config.get('stop_loss_compra_pct', 0.99)
    ...
    def _calcular_stop_gain_venda(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_gain_pct = config.get('stop_gain_venda_pct', 0.985)
    ...
    def _calcular_stop_loss_venda(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_loss_pct = config.get('stop_loss_venda_pct', 1.01)
    ...
    def _calcular_stop_gain_venda_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_gain_pct = config.get('stop_gain_venda_ind_pct', 0.985)
    ...
    def _calcular_stop_loss_venda_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_loss_pct = config.get('stop_loss_venda_ind_pct', 1.01)
    ...
    def _calcular_stop_gain_compra_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_gain_pct = config.get('stop_gain_compra_ind_pct', 1.015)
    ...
    def _calcular_stop_loss_compra_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_loss_pct = config.get('stop_loss_compra_ind_pct', 0.99)
...
# NÃO RECRIE o objeto trading_system automaticamente! Só crie se não existir.
...
if backend_disponivel and hasattr(sis, 'sistema_integrado') and sis.sistema_integrado:
    # Exibe status do sistema integrado de acordo com o clique do botão
    try:
        if hasattr(sis.sistema_integrado, 'is_analysis_running'):
            is_running = sis.sistema_integrado.is_analysis_running()
            # Só mostra "análise rodando" se realmente está ativo
            if is_running:
                st.sidebar.success('🧩 Sistema Integrado ativo (análise rodando)')
            else:
                st.sidebar.info('🧩 Sistema Integrado disponível (pronto para análise)')
        else:
            st.sidebar.info('🧩 Sistema Integrado disponível')
    except Exception as e:
    ...
    if not hasattr(sistema, 'sistema_integrado') or sistema.sistema_integrado is None:
        ...
        if hasattr(sistema, 'criar_sistema_integrado'):
            try:
                ...
                # Log erro sem exibir na interface (para evitar spam)
    ...
    try:
        ...
        if hasattr(sistema.sistema_integrado, 'is_analysis_running'):
            ...
            try:
                ...
                # Método existe mas não funciona, recria
                sistema.criar_sistema_integrado()
                return getattr(sistema, 'sistema_integrado', None)
        else:
            # Backend inválido, recria
            sistema.criar_sistema_integrado()
            return getattr(sistema, 'sistema_integrado', None)
    except Exception as e:
        # Backend com problema, recria
        try:
            sistema.criar_sistema_integrado()
            return getattr(sistema, 'sistema_integrado', None)
        except Exception as e2:
            print(f"[DEBUG] Erro ao recriar backend: {str(e2)}")
            return None
...
def render_sidebar():
   ...
   # else:
    ...
    # Verifica se sistema está rodando (principal OU análise específica)
    try:
        sistema_principal_running = sistema_integrado.running if backend_ok else False
        analysis_running = sistema_integrado.is_analysis_running() if backend_ok else False
        # Sistema está "rodando" se pelo menos o principal estiver ativo
        sistema_ativo = sistema_principal_running or analysis_running
    except Exception as e:
        st.warning(f"Erro ao verificar status do sistema: {str(e)}")
        sistema_principal_running = False
    ...
    col_btn, col_status = st.sidebar.columns([1, 1])
    with col_btn:
        # Botão para forçar aplicação imediata dos parâmetros
        if getattr(st.session_state, 'parametros_alterados', False):
            if st.button("🔄 Aplicar Parâmetros Agora", use_container_width=True, help="Força aplicação imediata dos parâmetros no sistema"):
                try:
                    from parametros_dinamicos import verificar_parametros_alterados, obter_config_sistema_principal
                    import time
                    if verificar_parametros_alterados():
                        # Obtém configuração atual antes de aplicar
                        config_nova = obter_config_sistema_principal()
                        # Aplica no sistema integrado se disponível
                        if hasattr(st.session_state.trading_system, 'sistema_integrado') and st.session_state.trading_system.sistema_integrado:
                            st.session_state.trading_system.sistema_integrado.aplicar_parametros_dinamicos()
                            # Aguarda até 3 segundos para thread realmente aplicar os parâmetros
                            max_wait = 3.0
                            waited = 0.0
                            check_interval = 0.15
                            sistema_integrado = st.session_state.trading_system.sistema_integrado
                            while waited < max_wait:
                                try:
                                    # Checa se análise está rodando após aplicar parâmetro (opcional)
                                    if hasattr(sistema_integrado, 'is_analysis_running') and sistema_integrado.is_analysis_running():
                                        break
                                except Exception:
                                    pass
                            ...
                            # Feedback detalhado
                            st.success("✅ Parâmetros aplicados imediatamente no sistema!")
                            st.info("📊 Próximos cálculos usarão os novos valores:")
                            col1, col2 = st.columns(2)
                            with col1:
                                st.write(f"• Limite operações: **{config_nova.get('limite_operacoes', 'N/A')}**")
                                st.write(f"• Valor operação: **R$ {config_nova.get('valor_operacao', 0):,}**")
                            with col2:
                                st.write(f"• Limite lucro: **{config_nova.get('limite_lucro', 'N/A')}**")
                                st.write(f"• R² mínimo: **{config_nova.get('r2_min', 'N/A')}**")
                            st.session_state.parametros_alterados = False
                            st.rerun()
                        else:
                            st.warning("⚠️ Sistema integrado não disponível - parâmetros salvos para próxima execução")
                            st.session_state.parametros_alterados = False
                            st.rerun()
                    else:
                        st.info("ℹ️ Não há parâmetros pendentes para aplicar")
                        st.session_state.parametros_alterados = False
                        st.rerun()
                except Exception as e:
                    st.error(f"❌ Erro ao aplicar parâmetros: {str(e)}")
        ...
        if not mt5_ok:
            ...
            # Sistema rodando (principal OU análise) - botão para parar
            if st.button("Parar Análise", use_container_width=True, help="Interrompe o sistema em background"):
                try:
                    if sistema_principal_running:
                        sistema_integrado.parar_sistema()
                        st.success("Sistema principal parado.")
                    elif analysis_running:
                        sistema_integrado.stop_analysis_thread()
                        st.success("Análise específica parada.")
                    st.rerun()
                except Exception as e:
            ...
            if st.button("Iniciar Análise", use_container_width=True, help="Inicia a análise em background"):
                try:
                    st.info("🚀 Iniciando sistema integrado completo...")
                    
                    # PRIMEIRO: Verifica se o sistema principal está rodando
                    if not sistema_integrado.running:
                        # Inicia o sistema principal COM TODAS AS THREADS
                        resultado_threads = sistema_integrado.iniciar_threads_apenas()
                        
                        if not resultado_threads:
                            st.error("❌ Falha ao iniciar sistema integrado principal")
                            return
                        ...
                        if sistema_integrado.running:
                            st.success("✅ Sistema integrado iniciado com sucesso!")
                        else:
                            st.error("❌ Sistema integrado falhou ao iniciar")
                            return
                    else:
                        st.info("ℹ️ Sistema integrado já está rodando")
                    
                    # SEGUNDO: Inicia a análise específica com parâmetros do dashboard
                    tabela_linha_operacao01 = getattr(st.session_state.trading_system, 'tabela_linha_operacao01', None)
                    
                    # ETAPA 4: Configuração robusta de parâmetros
                    ...
                    # Inicia a análise adicional com os parâmetros corretos (opcional)
                    if tabela_linha_operacao01 is not None and not tabela_linha_operacao01.empty:
                        started = sistema_integrado.start_analysis_thread(
                            tabela_linha_operacao01=tabela_linha_operacao01,
                            config=config_temp
                        )
                        # Aguarda até 3 segundos para thread realmente iniciar
                        import time
                        max_wait = 3.0
                        waited = 0.0
                        check_interval = 0.15
                        while waited < max_wait:
                            try:
                                if sistema_integrado.is_analysis_running():
                                    break
                            except Exception:
                                pass
                            time.sleep(check_interval)
                            waited += check_interval
                        if sistema_integrado.is_analysis_running():
                            st.success("✅ Análise específica iniciada com parâmetros atualizados!")
                        else:
                            st.error("❌ Falha ao iniciar análise: a thread não ficou ativa.")
                    else:
                        st.info("ℹ️ Sistema principal iniciado. Análise específica será executada quando dados estiverem disponíveis")
                    
                    # LOG DETALHADO PARA DEBUG
                    st.session_state["last_analysis_start"] = {
                        "sistema_principal_running": sistema_integrado.running,
                        "backend_ok": backend_ok,
                        "config_usado": config_temp,
                        "tabela_disponivel": tabela_linha_operacao01 is not None,
    ...
    with col_status:
        ...
        if not mt5_ok:
            ...
            # Sistema ativo - mostra qual tipo está rodando
            status_text = "Sistema Principal" if sistema_principal_running else "Análise Específica"
            st.markdown(f"""
    ...
    # Salva parâmetros dinamicamente para o sistema principal
    try:
        from parametros_dinamicos import salvar_config_dashboard
        
        # Verifica se houve alterações nos parâmetros
        parametros_anteriores = getattr(st.session_state, 'config_anterior', {})
        
        if parametros_anteriores != config_final:
            # Salva nova configuração
            sucesso = salvar_config_dashboard(config_final)
            
            if sucesso:
                sistema = st.session_state.trading_system
                if hasattr(sistema, 'log'):
                    sistema.log("🔄 Parâmetros atualizados - Sistema principal aplicará no próximo ciclo")
                    sistema.log(f"📊 Novos valores: max_posicoes={config_final.get('max_posicoes')}, valor_op={config_final.get('valor_operacao')}")
                
                # Armazena configuração atual para comparação futura
                st.session_state.config_anterior = config_final.copy()
                
                # Adiciona indicador visual de alteração
                st.session_state.parametros_alterados = True
                st.session_state.timestamp_alteracao = datetime.now()
                
                # Exibe notificação visual de alteração
                st.sidebar.success("🔄 Parâmetros atualizados! O sistema principal aplicará no próximo ciclo.")
                st.sidebar.info(f"⏰ Alteração em: {datetime.now().strftime('%H:%M:%S')}")
            
    except ImportError:
        st.sidebar.warning("⚠️ Sistema de parâmetros dinâmicos não disponível")
    except Exception as e:
        # Log do erro sem interromper o dashboard
        sistema = st.session_state.trading_system
        if hasattr(sistema, 'log'):
            sistema.log(f"⚠️ Erro ao salvar parâmetros dinâmicos: {e}")
        st.sidebar.error(f"❌ Erro ao salvar parâmetros: {str(e)}")
    
    # Mostra status dos parâmetros se foram alterados
    if getattr(st.session_state, 'parametros_alterados', False):
        tempo_alteracao = getattr(st.session_state, 'timestamp_alteracao', datetime.now())
        tempo_decorrido = (datetime.now() - tempo_alteracao).total_seconds()
        
        if tempo_decorrido < 300:  # 5 minutos
            st.sidebar.info(f"📊 Parâmetros alterados há {int(tempo_decorrido)}s - Aguardando próximo ciclo")
        else:
            # Remove o indicador após 5 minutos
            st.session_state.parametros_alterados = False
