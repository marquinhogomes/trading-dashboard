def atualizar_parametros(self, config):
    ...
    Pode ser chamado a qualquer momento para aplicar novos parâmetros sem reiniciar o sistema.
    """
    # Parâmetros principais
    self.valor_operacao = config.get('valor_operacao', self.valor_operacao)
    self.valor_operacao_ind = config.get('valor_operacao_ind', self.valor_operacao_ind)
    self.limite_lucro = config.get('limite_lucro', self.limite_lucro)
    self.limite_prejuizo = config.get('limite_prejuizo', self.limite_prejuizo)
    self.finaliza_ordens = config.get('finaliza_ordens', self.finaliza_ordens)
    self.ajusta_ordens = config.get('ajusta_ordens', self.ajusta_ordens)
    self.ajusta_ordens_minuto = config.get('ajusta_ordens_minuto', self.ajusta_ordens_minuto)
    self.horario_ajuste_stops = config.get('horario_ajuste_stops', self.horario_ajuste_stops)
    self.horario_remove_pendentes = config.get('horario_remove_pendentes', self.horario_remove_pendentes)
    self.horario_fechamento_total = config.get('horario_fechamento_total', self.horario_fechamento_total)
    self.inicia_pregao = config.get('inicia_pregao', getattr(self, 'inicia_pregao', 9))
    self.finaliza_pregao = config.get('finaliza_pregao', getattr(self, 'finaliza_pregao', 18))
    # Parâmetros de análise
    self.max_posicoes = config.get('max_posicoes', getattr(self, 'max_posicoes', 6))
    self.zscore_min = config.get('zscore_min', getattr(self, 'zscore_min', 2.0))
    self.zscore_max = config.get('zscore_max', getattr(self, 'zscore_max', 6.5))
    self.filtro_cointegracao = config.get('filtro_cointegracao', getattr(self, 'filtro_cointegracao', True))
    self.filtro_r2 = config.get('filtro_r2', getattr(self, 'filtro_r2', True))
    self.filtro_beta = config.get('filtro_beta', getattr(self, 'filtro_beta', True))
    self.filtro_zscore = config.get('filtro_zscore', getattr(self, 'filtro_zscore', True))
    self.r2_min = config.get('r2_min', getattr(self, 'r2_min', 0.5))
    self.beta_max = config.get('beta_max', getattr(self, 'beta_max', 2.0))
    self.coint_pvalue_max = config.get('coint_pvalue_max', getattr(self, 'coint_pvalue_max', 0.05))
    self.ativos_selecionados = config.get('ativos_selecionados', getattr(self, 'ativos_selecionados', []))
    self.timeframe = config.get('timeframe', getattr(self, 'timeframe', '1 dia'))
    self.periodo_analise = config.get('periodo_analise', getattr(self, 'periodo_analise', 120))
    self.usar_multiplos_periodos = config.get('usar_multiplos_periodos', getattr(self, 'usar_multiplos_periodos', True))
    self.intervalo_execucao = config.get('intervalo_execucao', getattr(self, 'intervalo_execucao', 60))
    # Log para depuração
    self.log(f"🔄 Parâmetros do sistema atualizados em tempo real via dashboard: {config}")
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class SistemaIntegrado:
    ...
    def __init__(self):
        ...
        # **ETAPA 1: NÃO inicializa threads automaticamente - evita loop infinito no Streamlit**
        print("[DEBUG] SistemaIntegrado.__init__ INICIADO")
        # self.iniciar_threads_apenas()  # DESABILITADO - threads serão iniciadas apenas quando necessário
    ...
    def executar_sistema_original(self):
        ...
        try:
            # SOLUÇÃO DEFINITIVA: Importa diretamente o módulo e chama a função main
            import sys
            import os
            ...
            # Importa o módulo calculo_entradas_v55
            import calculo_entradas_v55
            
            # Chama a função main uma única vez com loop=False
            self.log("🔄 Executando calculo_entradas_v55.main(loop=False)")
            
            # Executa a função main sem loop infinito
            ...
            # --- Correção crítica: copiar variáveis globais para atributos de instância ---
            # Acessa as variáveis globais do módulo importado
        ...
        # SOLUÇÃO DEFINITIVA: Garante que as tabelas estão sempre disponíveis
    ...
    def thread_sistema_trading(self):
        ...
        ultimo_execucao = 0
        intervalo_execucao = 300  # 5 minutos em segundos
    ...
    def thread_analise_e_envio_ordens(self):
        ...
        while self.running:
            try:
                ...
                if 9 <= current_hour < 17:  # 9h às 17h
                    # CORREÇÃO: Executa o sistema original apenas uma vez a cada 5 minutos
                    if (ultima_execucao is None or 
                        (agora - ultima_execucao).total_seconds() >= 300):  # 5 minutos
                        
                        self.log(f"📊 Executando análise às {current_hour:02d}:{current_minute:02d}")
                        
                        # Executa o sistema original de forma controlada (uma única vez)
                        self.executar_sistema_original()
                        ...
                        self.log(f"✅ Análise concluída - próxima execução em 5 minutos")
                    else:
                        # Calcula tempo restante
                        tempo_restante = 300 - (agora - ultima_execucao).total_seconds()
                        self.log(f"⏰ Próxima análise em {tempo_restante/60:.1f} minutos")
                
                # Aguarda próximo ciclo (1 minuto para verificação)
    ...
    def obter_operacoes_candidatas(self):
        ...
        """
        try:
            self.log("🔍 BUSCANDO: Operações candidatas da tabela_linha_operacao01 (nome correto)")
            # OPÇÃO 1: Extração direta da variável global tabela_linha_operacao01
    ...
    def executar_analise_e_envio_ordens(self, tabela_linha_operacao01=None, config=None):
        ...
        Adicionado log detalhado do campo 'ID' (valor e tipo) em cada etapa do processamento.
        """
        try:
    ...
    def aplicar_parametros_dinamicos(self):
        """Aplica parâmetros dinâmicos compartilhados com dashboard e sistema principal"""
        try:
            ...
            # Verifica se há parâmetros alterados
            if verificar_parametros_alterados():
                self.log("="*80)
                self.log("🔄 PARÂMETROS ALTERADOS DETECTADOS - APLICANDO NOVA CONFIGURAÇÃO")
                self.log("="*80)
                ...
                valores_antigos = {
                    'limite_operacoes': self.limite_operacoes,
                    'valor_operacao': self.valor_operacao,
                    'limite_lucro': self.limite_lucro,
                    'limite_prejuizo': self.limite_prejuizo,
                    'zscore_min': getattr(self, 'zscore_min', None),
                    'zscore_max': getattr(self, 'zscore_max', None),
                    'r2_min': getattr(self, 'r2_min', None),
                    'beta_max': getattr(self, 'beta_max', None),
                    'pvalor': getattr(self, 'pvalor', None),
                    'finaliza_ordens': getattr(self, 'finaliza_ordens', None),
                    'ajusta_ordens': getattr(self, 'ajusta_ordens', None),
                    'ajusta_ordens_minuto': getattr(self, 'ajusta_ordens_minuto', None),
                    'horario_ajuste_stops': getattr(self, 'horario_ajuste_stops', None),
                    'horario_remove_pendentes': getattr(self, 'horario_remove_pendentes', None),
                    'horario_fechamento_total': getattr(self, 'horario_fechamento_total', None)
                }

                # Atualiza parâmetros do sistema integrado
                self.limite_operacoes = config_dinamica.get('limite_operacoes', self.limite_operacoes)
                self.indep_limite_operacoes = config_dinamica.get('limite_operacoes', self.indep_limite_operacoes)
                self.valor_operacao = config_dinamica.get('valor_operacao', self.valor_operacao)
                self.valor_operacao_ind = config_dinamica.get('valor_operacao_ind', self.valor_operacao_ind)
                self.limite_lucro = config_dinamica.get('limite_lucro', self.limite_lucro)
                self.limite_prejuizo = config_dinamica.get('limite_prejuizo', self.limite_prejuizo)
                self.pvalor = config_dinamica.get('pvalor', self.pvalor)
                self.finaliza_ordens = config_dinamica.get('finaliza_ordens', self.finaliza_ordens)
                self.ajusta_ordens = config_dinamica.get('ajusta_ordens', self.ajusta_ordens)
                self.ajusta_ordens_minuto = config_dinamica.get('ajusta_ordens_minuto', self.ajusta_ordens_minuto)
                self.horario_ajuste_stops = config_dinamica.get('horario_ajuste_stops', self.horario_ajuste_stops)
                self.horario_remove_pendentes = config_dinamica.get('horario_remove_pendentes', self.horario_remove_pendentes)
                self.horario_fechamento_total = config_dinamica.get('horario_fechamento_total', self.horario_fechamento_total)
                # Parâmetros de análise/filtros
                ...
                # Log detalhado dos parâmetros aplicados
                self.log("🔄 REGENERANDO TABELAS COM NOVOS PARÂMETROS")
                self.log("[CONFIG] Tabelas marcadas como regeneradas")
                self.log("✅ Tabelas marcadas para regeneração")
                self.log("✅ NOVOS PARÂMETROS APLICADOS:")
                self.log(f"   • Período: {config_dinamica.get('periodo', 'N/A')}")
                self.log(f"   • Limite operações: {self.limite_operacoes}")
                self.log(f"   • Valor operação: R$ {self.valor_operacao:,}")
                self.log(f"   • Valor operação indep: R$ {self.valor_operacao_ind:,}")
                self.log(f"   • Limite lucro: R$ {self.limite_lucro}")
                self.log(f"   • Limite prejuízo: {self.limite_prejuizo}")
                self.log(f"   • Z-Score mínimo: {self.zscore_min}")
                self.log(f"   • Z-Score máximo: {self.zscore_max}")
                self.log(f"   • R² mínimo: {self.r2_min}")
                self.log(f"   • Beta máximo: {self.beta_max}")
                self.log(f"   • P-value ADF: {self.pvalor}")
                self.log(f"   • Finaliza ordens: {self.finaliza_ordens}h")
                self.log(f"   • Ajusta ordens: {self.ajusta_ordens}h")
                self.log(f"   • Ajusta ordens minuto: {self.ajusta_ordens_minuto}")
                self.log(f"   • Horário ajuste stops: {self.horario_ajuste_stops}")
                self.log(f"   • Horário remove pendentes: {self.horario_remove_pendentes}")
                self.log(f"   • Horário fechamento total: {self.horario_fechamento_total}")
                ...
                valores_novos = {
                    ...
                    'zscore_max': self.zscore_max,
                    'r2_min': self.r2_min,
                    'beta_max': self.beta_max,
                    'pvalor': self.pvalor,
                    'finaliza_ordens': self.finaliza_ordens,
                    'ajusta_ordens': self.ajusta_ordens,
                    'ajusta_ordens_minuto': self.ajusta_ordens_minuto,
                    'horario_ajuste_stops': self.horario_ajuste_stops,
                    'horario_remove_pendentes': self.horario_remove_pendentes,
                    'horario_fechamento_total': self.horario_fechamento_total
                }
                alteracoes = []
                for chave, valor_novo in valores_novos.items():
                    valor_antigo = valores_antigos.get(chave, None)
                    if valor_antigo != valor_novo:
                        alteracoes.append(f"   • {chave}: {valor_antigo} → {valor_novo}")
                if alteracoes:
                    self.log("📊 ALTERAÇÕES DETECTADAS:")
                    for alteracao in alteracoes:
                        self.log(alteracao)
                    self.log("🔄 Próximos cálculos usarão os novos valores!")
                else:
                    self.log("📊 Parâmetros aplicados mas sem alterações nos valores principais")

                self.log("="*80)
                # Marca como aplicado
                aplicar_parametros_sistema()
                self.log("[CONFIG] Parâmetros marcados como aplicados pelo sistema principal")
                self.log("[CONFIG] Flag de regeneração de tabelas ativado")
                self.log("✅ Parâmetros marcados como aplicados")

            # Verifica se as tabelas precisam ser regeneradas
                ...
                self.log("📊 Parâmetros dinâmicos verificados - sem alterações pendentes")

        except ImportError:
            self.log("⚠️ Sistema de parâmetros dinâmicos não disponível - usando configuração hardcoded")
        except Exception as e:
