...
try:
    # Tenta importar configurações do sistema original
    from calculo_entradas_v55 import (
        FINALIZA_ORDENS_HORA,
        PREFIXO_MAGIC_SCRIPT, 
        LIMITE_MAX_OPERACOES
    )
    # Se o import funcionar, usa as configurações originais
    FINALIZA_ORDENS = FINALIZA_ORDENS_HORA
    PREFIXO_SCRIPT = PREFIXO_MAGIC_SCRIPT
    LIMITE_OPERACOES = LIMITE_MAX_OPERACOES
except ImportError:
    # Fallback: se não conseguir importar, usa valores padrão baseados no original
    FINALIZA_ORDENS = 24  # Baseado no calculo_entradas_v55.py linha 1847
    PREFIXO_SCRIPT = "2"  # Baseado no calculo_entradas_v55.py linha 2104
    LIMITE_OPERACOES = 6  # Baseado no calculo_entradas_v55.py linha 2108
...
class TradingSystemReal:
    ...
    def _processar_compra_dep_venda_ind_debug(self, linha_selecionada, depende_atual, independe_atual, 
                     zscore_hoje, beta_hoje, r2_hoje, valor_operacao):
        """Versão com debug detalhado da função de compra DEP + venda IND - COM PROTEÇÃO TOTAL CONTRA None"""
        try:
            self.log(f"🟢 INICIANDO: COMPRA {depende_atual} + VENDA {independe_atual}")
            
            # ===== ETAPA 1: VALIDAÇÕES PRÉVIAS OBRIGATÓRIAS =====
            
            # 1.1 Verificação de horário (baseado no calculo_entradas_v55.py)
            current_hour = datetime.now().hour
            finaliza_ordens = FINALIZA_ORDENS  # Configurable: horário limite para envio de ordens
            
            if current_hour >= finaliza_ordens:
                self.log(f"❌ HORÁRIO LIMITE: Não é possível enviar ordens após {finaliza_ordens}h (atual: {current_hour}h)")
                self.log(f"🔧 REFERÊNCIA: calculo_entradas_v55.py FINALIZA_ORDENS_HORA = {FINALIZA_ORDENS}")
                return False
            
            # 1.2 MAGIC ID usando função centralizada
            magic_id = self.obter_magic_id_correto(depende_atual, independe_atual, linha_selecionada)
            if magic_id is None:
            ...
            prefixo_script = PREFIXO_SCRIPT  # Baseado no código original
            qtd_op_script = self._contar_operacoes_por_prefixo(prefixo_script)
            limite_operacoes = LIMITE_OPERACOES  # Configurable: limite máximo de operações
            ...
            if qtd_op_script >= limite_operacoes:
                ...
                self.log(f"🔧 REFERÊNCIA: calculo_entradas_v55.py LIMITE_MAX_OPERACOES = {LIMITE_OPERACOES}")
            ...
            # ===== CORREÇÃO CRÍTICA: NORMALIZAÇÃO COMPLETA DE TODAS AS VARIÁVEIS =====
            
            # ✅ PROTEÇÃO TOTAL: Normalização segura de TODAS as variáveis de entrada
            try:
                ...
                valor_operacao = float(valor_operacao) if valor_operacao is not None else 10000.0
            except (ValueError, TypeError) as e:
                self.log(f"❌ ERRO: Parâmetros de entrada inválidos: {str(e)}")
                return False
            
            self.log(f"🔧 DEBUG: Parâmetros normalizados - zscore={zscore_hoje}, beta={beta_hoje}, r2={r2_hoje}")
            
            # 2.2 Extrair variáveis técnicas da tabela - COM PROTEÇÃO TOTAL
            preco_atuall = linha_selecionada.get('preco_atual', symbol_info_tick_dep.ask)
            
            # ✅ PROTEÇÃO: Normalização segura do preço atual
            try:
                preco_atuall = float(preco_atuall) if preco_atuall is not None else float(symbol_info_tick_dep.ask)
            except (ValueError, TypeError):
                preco_atuall = float(symbol_info_tick_dep.ask)
            
            # ✅ CORREÇÃO: Busca por diferentes nomes possíveis da variável residual COM PROTEÇÃO
            pred_resid = None
            resid_atual = None
            ...
            for key_name in ['pred_resid', 'pred_residual', 'predicted_residual', 'residuo_previsto']:
                if key_name in linha_selecionada:
                    try:
                        valor = linha_selecionada[key_name]
                        if valor is not None:
                            pred_resid = float(valor)
            ...
            for key_name in ['resid_atual', 'residual_atual', 'current_residual', 'residuo_atual', 'residuo_std', 'residuo']:
                if key_name in linha_selecionada:
                    try:
                        valor = linha_selecionada[key_name]
                        if valor is not None:
                            resid_atual = float(valor)
                            self.log(f"🔧 DEBUG: resid_atual encontrado como '{key_name}': {resid_atual}")
                            break
                    except (ValueError, TypeError):
                        continue
            
            # ✅ PROTEÇÃO TOTAL: Garantir que valores não sejam None
            ...
            if pred_resid == 0.0 and resid_atual == 0.0 and abs(zscore_hoje) > 0.1:
                # Para compra: pred_resid deve ser maior que resid_atual
                resid_atual = abs(zscore_hoje) * 0.5  # Valor atual do resíduo
                pred_resid = resid_atual * 1.2       # Previsão maior (sinal de compra)
            ...
            # 2.3 Verificar se já existe ordem de compra pendente para o ativo dependente
            existe_ordem_compra = self._verificar_operacao_aberta_tipo(depende_atual, 'buy')
            
            if existe_ordem_compra:
                self.log(f"❌ ORDEM COMPRA EXISTENTE: Já existe ordem de compra para {depende_atual}")
                return False
            
            # 2.4 Calcular preços de entrada COM PROTEÇÃO
            ...
            try:
                ...
                # ✅ PROTEÇÃO CRÍTICA: Validação dos preços calculados
                ...
                # ✅ PROTEÇÃO: Normalização segura dos preços calculados
                price_dep_compra = float(preco_compra_dep)
                price_ind_venda = float(preco_venda_ind)
                
                # ✅ VALIDAÇÃO: Preços devem ser positivos
                if price_dep_compra <= 0 or price_ind_venda <= 0:
                    self.log(f"❌ ERRO: Preços calculados são inválidos (≤0): compra={price_dep_compra}, venda={price_ind_venda}")
                    return False
                    
            except (ValueError, TypeError, AttributeError) as e:
                self.log(f"❌ ERRO: Falha ao calcular/normalizar preços: {str(e)}")
                return False
            
            self.log(f"✅ Preços calculados: COMPRA {depende_atual}=R${price_dep_compra:.2f}, VENDA {independe_atual}=R${price_ind_venda:.2f}")
            
            # ===== ETAPA 3: CÁLCULO DE STOPS COM PROTEÇÃO TOTAL =====
            
            self.log(f"🎯 Calculando stops para determinar min_dist_acao_dep...")
            
            try:
                # Stops do dependente da tabela
                spread_compra_gain_original = linha_selecionada.get('spread_compra_gain')
                if spread_compra_gain_original is not None:
                    stop_gain_compra = float(spread_compra_gain_original)
                    self.log(f"✅ Stop Gain compra DEP da tabela: R$ {stop_gain_compra:.2f}")
                else:
                    stop_gain_compra = self._calcular_stop_gain_compra(linha_selecionada, price_dep_compra)
                    self.log(f"⚠️ Stop Gain compra DEP calculado: R$ {stop_gain_compra:.2f}")
                
                # ✅ PROTEÇÃO: Garantir que stop_gain_compra não seja None
                if stop_gain_compra is None:
                    stop_gain_compra = price_dep_compra * 1.015  # Fallback: 1.5% acima
                    self.log(f"🔧 Stop Gain fallback aplicado: R$ {stop_gain_compra:.2f}")
                
                stop_gain_compra = float(stop_gain_compra)
                
                # ✅ CORREÇÃO BASEADA NO CÓDIGO ORIGINAL: Para COMPRA DEP + VENDA IND
                # min_dist_acao_dep = stop_gain * 0.99 (baseado no calculo_entradas_v55.py)
                min_dist_acao_dep_numeric = stop_gain_compra * 0.99
                
                self.log(f"📏 Min distância ação dep calculada: R$ {min_dist_acao_dep_numeric:.2f} (stop_gain * 0.99)")
                
            except (ValueError, TypeError, AttributeError) as e:
                self.log(f"❌ ERRO ao calcular stops: {str(e)}")
                return False
            
            # ===== ETAPA 4: VALIDAÇÕES TÉCNICAS CRÍTICAS COM PROTEÇÃO TOTAL =====
            
            self.log(f"🔍 Executando validações técnicas críticas...")
            
            # ✅ CORREÇÃO CRÍTICA: Validação com proteção total contra None e tipos inválidos
            
            try:
                # 4.1 Validação: pred_resid > resid_atual (para compra) - COM TOLERÂNCIA E PROTEÇÃO TOTAL
                ...
                if not condicao_residuo:
                    ...
                    if pred_resid == 0.0 and resid_atual == 0.0:
                        if zscore_hoje <= -2.0:
                            self.log(f"✅ EXCEÇÃO: Resíduos zerados, mas Z-Score válido para compra: {zscore_hoje:.3f}")
                            condicao_residuo = True
                        else:
                            self.log(f"❌ CONDIÇÃO TÉCNICA FALHOU: Resíduos zerados e Z-Score insuficiente: {zscore_hoje}")
                            return False
                    else:
                        self.log(f"❌ CONDIÇÃO TÉCNICA FALHOU: pred_resid ({pred_resid:.4f}) não é maior que resid_atual ({resid_atual:.4f})")
                        return False
                
                # 4.2 Validação: price_dep_compra < min_dist_acao_dep (menor para compra) - COM PROTEÇÃO TOTAL
                condicao_distancia = price_dep_compra < min_dist_acao_dep_numeric
                
                self.log(f"🔍 Condição 2 - price_dep_compra < min_dist_acao_dep: {price_dep_compra:.2f} < {min_dist_acao_dep_numeric:.2f} = {condicao_distancia}")
                
                if not condicao_distancia:
                    self.log(f"❌ CONDIÇÃO TÉCNICA FALHOU: price_dep_compra ({price_dep_compra:.2f}) não é menor que min_dist_acao_dep ({min_dist_acao_dep_numeric:.2f})")
                    return False
                
                # 4.3 Validação: price_dep_compra < preco_atuall (menor para compra) - COM PROTEÇÃO TOTAL
                condicao_preco_atual = price_dep_compra < preco_atuall
                self.log(f"🔍 Condição 3 - price_dep_compra < preco_atual: {price_dep_compra:.2f} < {preco_atuall:.2f} = {condicao_preco_atual}")
                
                if not condicao_preco_atual:
                    self.log(f"❌ CONDIÇÃO TÉCNICA FALHOU: price_dep_compra ({price_dep_compra:.2f}) não é menor que preco_atual ({preco_atuall:.2f})")
                    return False
                
            except (ValueError, TypeError, AttributeError) as e:
                self.log(f"❌ ERRO na validação técnica: {str(e)}")
                return False
            
            # 4.4 TODAS AS CONDIÇÕES TÉCNICAS APROVADAS
            self.log(f"✅ TODAS AS VALIDAÇÕES TÉCNICAS APROVADAS!")
            self.log(f"   ✓ Resíduo: {pred_resid:.4f} > {resid_atual:.4f}")
            self.log(f"   ✓ Distância: {price_dep_compra:.2f} < {min_dist_acao_dep_numeric:.2f}")
            self.log(f"   ✓ Preço atual: {price_dep_compra:.2f} < {preco_atuall:.2f}")
            
            # ===== ETAPA 5: CÁLCULO DE STOPS E VOLUMES COM PROTEÇÃO =====
            
            try:
                # Calcula stops do dependente
                stop_loss_compra = self._calcular_stop_loss_compra(linha_selecionada, price_dep_compra)
                if stop_loss_compra is None:
                    stop_loss_compra = price_dep_compra * 0.99
                stop_loss_compra = float(stop_loss_compra)
                
                # Calcula stops do independente
                stop_gain_venda_ind = self._calcular_stop_gain_venda_ind(linha_selecionada, price_ind_venda)
                if stop_gain_venda_ind is None:
                    stop_gain_venda_ind = price_ind_venda * 0.985
                stop_gain_venda_ind = float(stop_gain_venda_ind)
                
                stop_loss_venda_ind = self._calcular_stop_loss_venda_ind(linha_selecionada, price_ind_venda)
                if stop_loss_venda_ind is None:
                    stop_loss_venda_ind = price_ind_venda * 1.01
                stop_loss_venda_ind = float(stop_loss_venda_ind)
                
                # Calcula volumes
                qtd_dep = self._calcular_volume_operacao(price_dep_compra, valor_operacao)
                qtd_ind = round(qtd_dep * abs(beta_hoje), -2)
                
                # ✅ PROTEÇÃO: Volumes devem ser positivos
                if qtd_dep <= 0 or qtd_ind <= 0:
                    self.log(f"❌ Volume inválido: DEP={qtd_dep}, IND={qtd_ind}")
                    return False
                    
            except (ValueError, TypeError, AttributeError) as e:
                self.log(f"❌ ERRO ao calcular stops/volumes: {str(e)}")
                return False
            
            self.log(f"🎯 VALORES FINAIS CALCULADOS:")
            self.log(f"   • Preço compra DEP: R$ {price_dep_compra:.2f}")
            self.log(f"   • Preço venda IND: R$ {price_ind_venda:.2f}")
            self.log(f"   • Stop Gain DEP: R$ {stop_gain_compra:.2f}")
            self.log(f"   • Stop Loss DEP: R$ {stop_loss_compra:.2f}")
            self.log(f"   • Volume DEP: {qtd_dep}")
            self.log(f"   • Volume IND: {qtd_ind}")
            
            # ===== ETAPA 6: LÓGICA DE ENVIO SEQUENCIAL E ROLLBACK =====
            ...
            ordem_venda_ind = {
                ...
                "magic": magic_id,
                "comment": f"OptInd_B{beta_hoje:.2f}",
                "type_time": mt5.ORDER_TIME_DAY,
                "type_filling": mt5.ORDER_FILLING_RETURN,
            }

            # Envio sequencial: DEPENDENTE PRIMEIRO
            self.log(f"📊 PASSO 1/2: Enviando ordem DEPENDENTE (COMPRA {depende_atual})")

            result_compra_dep = mt5.order_send(ordem_compra_dep)

            if result_compra_dep is None:
                self.log(f"❌ ERRO CRÍTICO: result_compra_dep retornou None (sem resposta do MT5)")
                self.log(f"❌ Erro MT5: {mt5.last_error()}")
                return False

            self.log(f"📋 DEPENDENTE - Retcode: {result_compra_dep.retcode}, Comentário: {result_compra_dep.comment}")

            # Validação do envio do dependente
            if result_compra_dep.retcode == mt5.TRADE_RETCODE_DONE:
                self.log(f"✅ DEPENDENTE enviado com sucesso - Ticket: {result_compra_dep.order}")
                ticket_dep = result_compra_dep.order

                # Envio do independente apenas se dependente foi aprovado
                self.log(f"📊 PASSO 2/2: Enviando ordem INDEPENDENTE (VENDA {independe_atual})")

                result_venda_ind = mt5.order_send(ordem_venda_ind)

                if result_venda_ind is None:
                    self.log(f"❌ ERRO CRÍTICO: result_venda_ind retornou None")
                    self.log(f"❌ Erro MT5: {mt5.last_error()}")

                    # ROLLBACK: Cancela dependente se independente falhou
                ...
                self.log(f"📋 INDEPENDENTE - Retcode: {result_venda_ind.retcode}, Comentário: {result_venda_ind.comment}")

                # Validação do envio do independente
                if result_venda_ind.retcode == mt5.TRADE_RETCODE_DONE:
                    self.log(f"✅ INDEPENDENTE enviado com sucesso - Ticket: {result_venda_ind.order}")

                    # Ambas as ordens aprovadas: registro do par
                    ...
                    detalhes_compra = {
                        ...
                        'tp_dep': stop_gain_compra,
                        'sl_dep': stop_loss_compra,
                        'Preco Independente': price_ind_venda,
                        'tp_ind': stop_gain_venda_ind,
                        'sl_ind': stop_loss_venda_ind,
                        'magic': magic_id,
                        'Timestamp': datetime.now(),
                        'ticket_dep': ticket_dep,
                        'ticket_ind': result_venda_ind.order,
                        'tipo_operacao': 'COMPRA_DEP_VENDA_IND'
                    }

                    if not hasattr(self, 'operacoes_executadas'):
                        self.operacoes_executadas = []

                    self.operacoes_executadas.append(detalhes_compra)
                    ...
                    self.log(f"   ├─ Ticket IND: {result_venda_ind.order}")
                    self.log(f"   └─ Z-Score: {zscore_hoje:.2f}")

                    return True

                else:
                    # Independente falhou: rollback do dependente
                    self.log(f"❌ INDEPENDENTE FALHOU: retcode={result_venda_ind.retcode}")
                    self.log(f"🔄 EXECUTANDO ROLLBACK: Cancelando ordem DEPENDENTE (ticket {ticket_dep})")
                ...
                # Dependente falhou: não envia independente
                self.log(f"❌ DEPENDENTE FALHOU: retcode={result_compra_dep.retcode}")
                self.log(f"🚫 OPERAÇÃO ABORTADA: Não enviando ordem INDEPENDENTE")
                return False
            
        except Exception as e:
            self.log(f"❌ Erro geral na função de compra DEP + venda IND: {str(e)}")
            self.log(f"🔧 DEBUG: Linha do erro: {traceback.format_exc()}")
            return False
                
    def _processar_venda_dep_compra_ind_debug(self, linha_selecionada, depende_atual, independe_atual,
                         zscore_hoje, beta_hoje, r2_hoje, valor_operacao):
        """Versão com debug detalhado da função de venda DEP + compra IND - COM VALIDAÇÕES PRÉVIAS E MAGIC ID CORRETO"""
        try:
