...
# Função utilitária para buscar parâmetro dinâmico centralizado
def get_parametro_dinamico(nome, fallback=None):
    # Busca robusta do sistema integrado (backend)
    sistema = None
    try:
        # 1. Tenta pelo globals()
        if 'sistema_integrado' in globals():
            sistema = globals()['sistema_integrado']
        # 2. Tenta pelo st.session_state (Streamlit)
        elif 'st' in globals() and hasattr(st, 'session_state') and 'trading_system' in st.session_state:
            sistema = getattr(st.session_state, 'trading_system', None)
        # 3. Tenta pelo TradingSystemReal (caso instanciado)
        elif 'TradingSystemReal' in globals():
            sistema = globals().get('TradingSystemReal', None)
    except Exception:
        sistema = None
    # Busca o atributo se possível
    if sistema and hasattr(sistema, nome):
        return getattr(sistema, nome)
    # Busca do módulo de cálculo
    if hasattr(calc_mod, nome):
        return getattr(calc_mod, nome)
    # Fallback
    return fallback
import numpy as np
...
# Inicializa last_auto_refresh se não existir (correção de bug)
...
if len(st.session_state.debug_auto_refresh) > 10:
    st.session_state.debug_auto_refresh = st.session_state.debug_auto_refresh[-10:]  # Limite de 10 para debug visual, não parâmetro de negócio

# Indicador de auto-refresh no canto superior direito
with st.container():
    col_main, col_refresh = st.columns([10, 1])  # Ajuste visual de layout, não parâmetro de negócio
    with col_refresh:
        if st.session_state.auto_refresh_enabled:
...
class TradingSystemReal:
    ...
    def __init__(self):
        ...
        # Inicializar estruturas de dados e logs PRIMEIRO, antes de qualquer operação
        self.dados_sistema = {
            "execucoes": 0,
            "pares_processados": 0,
            "ordens_enviadas": 0,
        ...
        self.sistema_integrado_status = 'nao_inicializado'
            ...
            # Inicializa o sistema integrado automaticamente se disponível
        if SISTEMA_INTEGRADO_DISPONIVEL:
            self.criar_sistema_integrado()

    def criar_sistema_integrado(self):
        """Inicializa o backend Sistema Integrado de forma segura e real."""
        try:
            print("[DEBUG] Criando sistema_integrado...")
            from sistema_integrado import SistemaIntegrado
            self.sistema_integrado = SistemaIntegrado()
            self.sistema_integrado_status = 'inicializado'
            print(f"[DEBUG] Sistema integrado criado com sucesso: {self.sistema_integrado}")
        except Exception as e:
            print(f"[DEBUG] ERRO ao criar sistema_integrado: {e}")
            import traceback
            print(f"[DEBUG] Traceback: {traceback.format_exc()}")
            self.sistema_integrado = None
            self.sistema_integrado_status = f'erro: {e}'
            if hasattr(self, 'log'):
                self.log(f"Erro ao criar sistema_integrado: {e}")
    ...
    def _calcular_stop_gain_venda(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_gain_pct = get_parametro_dinamico('stop_gain_venda_pct', 0.985)
    ...
    def _calcular_stop_loss_venda(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_loss_pct = get_parametro_dinamico('stop_loss_venda_pct', 1.01)
    ...
    def _calcular_stop_gain_venda_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_gain_pct = get_parametro_dinamico('stop_gain_venda_ind_pct', 0.985)
    ...
    def _calcular_stop_loss_venda_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_loss_pct = get_parametro_dinamico('stop_loss_venda_ind_pct', 1.01)
    ...
    def _calcular_stop_gain_compra_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_gain_pct = get_parametro_dinamico('stop_gain_compra_ind_pct', 1.015)
    ...
    def _calcular_stop_loss_compra_ind(self, linha_selecionada, preco_entrada):
        ...
        try:
            from streamlit import session_state
            config = session_state.get('config_final', {})
            stop_loss_pct = get_parametro_dinamico('stop_loss_compra_ind_pct', 0.99)
    ...
    def _validar_condicoes_entrada_compra(self, linha_selecionada, preco_compra):
        ...
        try:
            ...
            if preco_compra <= 0:
                self.log(f"[REPROVADO] Par não aceito para COMPRA: preço de compra inválido (<= 0). Linha: {linha_selecionada}")
                return False
            # Limiar Z-Score (compra: menor que -limiar)
            zscore_lim = -abs(get_parametro_dinamico('zscore_min', 2.0))
            zscore = linha_selecionada.get('Z-Score', 0)
            if zscore > zscore_lim:
                self.log(f"[REPROVADO] Par não aceito para COMPRA: Z-Score {zscore:.3f} > limiar {zscore_lim:.3f}. Linha: {linha_selecionada}")
                return False
            # Limiar R²
            r2_min = get_parametro_dinamico('r2_min', 0.3)
            r2 = linha_selecionada.get('r2', 0)
            if r2 < r2_min:
                self.log(f"[REPROVADO] Par não aceito para COMPRA: r² {r2:.3f} < limiar {r2_min:.3f}. Linha: {linha_selecionada}")
                return False
            # Limiar beta máximo
            beta_max = get_parametro_dinamico('beta_max', 2.0)
            beta = abs(linha_selecionada.get('beta', 0))
            if beta > beta_max:
                self.log(f"[REPROVADO] Par não aceito para COMPRA: |beta| {beta:.3f} > limiar {beta_max:.3f}. Linha: {linha_selecionada}")
                return False
            # Limiar p-valor de cointegração
            coint_pvalue_max = get_parametro_dinamico('coint_pvalue_max', 0.05)
            coint_p = linha_selecionada.get('coint_p_value', 1)
            if coint_p > coint_pvalue_max:
                self.log(f"[REPROVADO] Par não aceito para COMPRA: p-valor cointegração {coint_p:.4f} > limiar {coint_pvalue_max:.4f}. Linha: {linha_selecionada}")
                return False
            ...
            if pred_resid is not None and resid_atual is not None:
                if float(pred_resid) <= float(resid_atual):
                    self.log(f"[REPROVADO] Par não aceito para COMPRA: pred_resid {pred_resid} <= resid_atual {resid_atual}. Linha: {linha_selecionada}")
                    return False
            return True
        except Exception as e:
            self.log(f"[ERRO] Erro na validação de condições de compra: {e}. Linha: {linha_selecionada}")
            return False
    ...
    def _validar_condicoes_entrada_venda(self, linha_selecionada, preco_venda):
        ...
        try:
            ...
            # Validação básica de preço
            if preco_venda <= 0:
                self.log(f"[REPROVADO] Par não aceito para VENDA: preço de venda inválido (<= 0). Linha: {linha_selecionada}")
                return False
            # Limiar Z-Score (venda: maior que limiar)
            zscore_lim = abs(get_parametro_dinamico('zscore_min', 2.0))
            zscore = linha_selecionada.get('Z-Score', 0)
            if zscore < zscore_lim:
                self.log(f"[REPROVADO] Par não aceito para VENDA: Z-Score {zscore:.3f} < limiar {zscore_lim:.3f}. Linha: {linha_selecionada}")
                return False
            # Limiar R²
            r2_min = get_parametro_dinamico('r2_min', 0.3)
            r2 = linha_selecionada.get('r2', 0)
            if r2 < r2_min:
                self.log(f"[REPROVADO] Par não aceito para VENDA: r² {r2:.3f} < limiar {r2_min:.3f}. Linha: {linha_selecionada}")
                return False
            # Limiar beta máximo
            beta_max = get_parametro_dinamico('beta_max', 2.0)
            beta = abs(linha_selecionada.get('beta', 0))
            if beta > beta_max:
                self.log(f"[REPROVADO] Par não aceito para VENDA: |beta| {beta:.3f} > limiar {beta_max:.3f}. Linha: {linha_selecionada}")
                return False
            # Limiar p-valor de cointegração
            coint_pvalue_max = get_parametro_dinamico('coint_pvalue_max', 0.05)
            coint_p = linha_selecionada.get('coint_p_value', 1)
            if coint_p > coint_pvalue_max:
                self.log(f"[REPROVADO] Par não aceito para VENDA: p-valor cointegração {coint_p:.4f} > limiar {coint_pvalue_max:.4f}. Linha: {linha_selecionada}")
                return False
            # Validação de resíduo (se disponível)
            pred_resid = linha_selecionada.get('pred_resid')
            resid_atual = linha_selecionada.get('resid_atual')
            if pred_resid is not None and resid_atual is not None:
                if float(pred_resid) >= float(resid_atual):
                    self.log(f"[REPROVADO] Par não aceito para VENDA: pred_resid {pred_resid} >= resid_atual {resid_atual}. Linha: {linha_selecionada}")
                    return False
            return True
        except Exception as e:
            self.log(f"[ERRO] Erro na validação de condições de venda: {e}. Linha: {linha_selecionada}")
            return False
    ...
    def _criar_segunda_selecao_fallback(self):
        ...
        try:
            ...
            # Garante que todos os parâmetros relevantes sejam extraídos da sidebar/config
            zscore_min = config.get('zscore_min', 2.0)
            ...
            mascara_segunda = (
                (df_primeira['Z-Score'].abs() >= zscore_min) &
                (df_primeira['r2'] >= r2_min) &
                (df_primeira['beta'].abs() <= beta_max) &
                (df_primeira['coint_p_value'] <= coint_pvalue_max)
            )
            ...
            while self.running and self.sistema_integrado.running:
                try:
                    ...
                    if hasattr(self.sistema_integrado, 'logs') and self.sistema_integrado.logs:
                        novos_logs = self.sistema_integrado.logs[-10:]
                        for log in novos_logs:
                            if log not in self.logs[-10:]:
                    ...
                    if hasattr(self.sistema_integrado, 'dados_sistema'):
                        ...
                        if ordens_count > 0 or pares_sistema_count > 0:
                    ...
                    # Reforço: monitora threads críticas do sistema integrado
                    threads_criticas = [
                    ...
                    for nome_thread in threads_criticas:
                        thread_obj = getattr(self.sistema_integrado, nome_thread, None)
                        if thread_obj is not None and hasattr(thread_obj, 'is_alive'):
                            if not thread_obj.is_alive():
                                self.log(f"❌ Thread crítica '{nome_thread}' parou inesperadamente! Stack: {traceback.format_stack()}")
...
# ===== ETAPA 2: INICIALIZAÇÃO AUTOMÁTICA DO BACKEND =====
...
if 'trading_system' not in st.session_state or st.session_state.get('trading_system') is None:
    ...
    # Inicializa o backend integrado e threads apenas uma vez
    if hasattr(st.session_state.trading_system, 'sistema_integrado') and st.session_state.trading_system.sistema_integrado:
        if not getattr(st.session_state.trading_system.sistema_integrado, 'running', False):
            st.session_state.trading_system.sistema_integrado.iniciar_sistema()
else:
    # Se já existe, garante que as threads estão rodando (proteção extra)
    if hasattr(st.session_state.trading_system, 'sistema_integrado') and st.session_state.trading_system.sistema_integrado:
        if not getattr(st.session_state.trading_system.sistema_integrado, 'running', False):
            st.session_state.trading_system.sistema_integrado.iniciar_sistema()

# ETAPA 2: Garante que o backend (sistema_integrado) está sempre disponível
def garantir_backend_disponivel():
    """Garante que o backend Sistema Integrado está sempre disponível e inicializado"""
    sistema = st.session_state.trading_system
    
    # Verifica se o backend já está disponível
    if hasattr(sistema, 'sistema_integrado') and sistema.sistema_integrado is not None:
        # Backend já disponível, apenas log de debug
        if hasattr(sistema.sistema_integrado, 'is_analysis_running'):
            try:
                is_running = sistema.sistema_integrado.is_analysis_running()
    ...
    # Backend não disponível, precisa criar
    if hasattr(sistema, 'criar_sistema_integrado'):
        try:
            sistema.criar_sistema_integrado()
            # Verifica se foi criado com sucesso
            if hasattr(sistema, 'sistema_integrado') and sistema.sistema_integrado is not None:
                st.sidebar.success('🧩 Backend Sistema Integrado inicializado automaticamente')
                return True
            else:
                st.sidebar.error('❌ Falha ao inicializar backend Sistema Integrado')
                return False
        except Exception as e:
            st.sidebar.error(f'❌ Erro ao inicializar backend: {str(e)}')
            return False
    else:
        st.sidebar.error('❌ Método crear_sistema_integrado não encontrado')
        return False

# Executa a garantia de backend disponível
backend_disponivel = garantir_backend_disponivel()
...
# Exibe status do sistema integrado na interface de forma robusta
sis = st.session_state.trading_system
if backend_disponivel and hasattr(sis, 'sistema_integrado') and sis.sistema_integrado:
    # Verifica se as threads estão rodando
    try:
        if hasattr(sis.sistema_integrado, 'is_analysis_running'):
            is_running = sis.sistema_integrado.is_analysis_running()
            if is_running:
                st.sidebar.success('🧩 Sistema Integrado ativo (análise rodando)')
            else:
                st.sidebar.success('🧩 Sistema Integrado disponível (pronto para análise)')
        else:
            st.sidebar.success('🧩 Sistema Integrado disponível')
    except Exception as e:
        st.sidebar.warning(f'🧩 Sistema Integrado disponível (erro ao verificar status: {str(e)})')
else:
    status_msg = getattr(sis, "sistema_integrado_status", "desconhecido")
    st.sidebar.error(f'❌ Sistema Integrado não disponível: {status_msg}')
...
def verificar_backend_robusto():
    """
    ETAPA 3: Verifica e garante que o backend esteja disponível para uso
    Retorna o sistema_integrado se disponível, ou None se não disponível
    """
    sistema = st.session_state.trading_system

    # Persistência: só cria/recria o backend se realmente não existir ou estiver corrompido
    if not hasattr(sistema, 'sistema_integrado') or sistema.sistema_integrado is None:
        if hasattr(sistema, 'criar_sistema_integrado'):
            try:
                sistema.criar_sistema_integrado()
                if hasattr(sistema, 'sistema_integrado') and sistema.sistema_integrado is not None:
                    return sistema.sistema_integrado
            except Exception as e:
                print(f"[DEBUG] Erro ao criar backend: {str(e)}")
                return None
        return None

    # Segunda verificação: backend é válido?
    try:
        # Testa se o backend tem os métodos necessários
        if hasattr(sistema.sistema_integrado, 'is_analysis_running'):
            # Teste adicional: verifica se o método funciona
            try:
                sistema.sistema_integrado.is_analysis_running()
                # Backend válido e funcional
                return sistema.sistema_integrado
            except Exception as e:
                # Só recria se for erro crítico (AttributeError, ImportError, etc)
                import traceback
                print(f"[DEBUG] Erro ao testar backend: {e}\n{traceback.format_exc()}")
                # Só recria se backend realmente não funcionar mais
                if type(e) in (AttributeError, ImportError, RuntimeError):
                    if hasattr(sistema, 'criar_sistema_integrado'):
                        sistema.criar_sistema_integrado()
                        return getattr(sistema, 'sistema_integrado', None)
                return sistema.sistema_integrado
        else:
            # Backend inválido, só recria se nunca inicializado
            if not hasattr(sistema, 'sistema_integrado') or sistema.sistema_integrado is None:
                if hasattr(sistema, 'criar_sistema_integrado'):
                    sistema.criar_sistema_integrado()
                    return getattr(sistema, 'sistema_integrado', None)
        ...
        print(f"[DEBUG] Erro inesperado ao verificar backend: {e}")
        return sistema.sistema_integrado
...
def render_sidebar():
   ...
   # else:
    ...
    # --- CONTROLE DE THREADS DO SISTEMA INTEGRADO (ETAPA 3: VERIFICAÇÃO ROBUSTA) ---
    
    # Verifica backend de forma robusta
    sistema_integrado = verificar_backend_robusto()
    backend_ok = sistema_integrado is not None
    mt5_ok = st.session_state.trading_system.mt5_connected
    
    # Verifica se análise está rodando
    try:
        # CORREÇÃO: Usar método correto para verificar se sistema está rodando
        analysis_running = getattr(sistema_integrado, 'running', False) if backend_ok else False
    except Exception as e:
        st.warning(f"Erro ao verificar status da análise: {str(e)}")
        analysis_running = False
    
    # As verificações de análise já foram feitas acima na verificação robusta
    
    col_btn, col_status = st.sidebar.columns([1, 1])
    with col_btn:
        # ETAPA 4: Lógica corrigida dos botões com verificação robusta
        if not mt5_ok:
            # MT5 não conectado - botão desabilitado
            st.button("Iniciar Análise", use_container_width=True, help="Conecte ao MT5 primeiro", disabled=True)
        elif not backend_ok:
            # Backend não disponível - botão com erro
            st.button("Backend Indisponível", use_container_width=True, help="Erro no Sistema Integrado", disabled=True)
        elif analysis_running:
            # Análise rodando - botão para parar
            if st.button("Parar Análise", use_container_width=True, help="Interrompe a análise em background"):
                try:
                    # CORREÇÃO: Usar método correto para parar
                    if hasattr(sistema_integrado, 'parar_sistema'):
                        sistema_integrado.parar_sistema()
                        st.success("Análise parada.")
                    elif hasattr(sistema_integrado, 'running'):
                        sistema_integrado.running = False
                        st.success("Análise parada.")
                    else:
                        st.warning("Não foi possível parar a análise.")
                    st.rerun()
                except Exception as e:
                    st.error(f"Erro ao parar análise: {str(e)}")
        else:
            # Tudo OK - botão para iniciar análise
            if st.button("Iniciar Análise", use_container_width=True, help="Inicia a análise em background"):
                try:
                    # CORREÇÃO: Usar método correto para iniciar sistema integrado
                    ...
                    # CORREÇÃO: Usar método correto para iniciar threads
                    started = False
                    
                    # Opção 1: Método iniciar_threads_apenas (se existir)
                    if hasattr(sistema_integrado, 'iniciar_threads_apenas'):
                        started = sistema_integrado.iniciar_threads_apenas()
                        if started:
                            st.success("✅ Sistema integrado iniciado com sucesso!")
                        else:
                            st.error("❌ Falha ao iniciar sistema integrado")
                    
                    # Opção 2: Método executar_sistema_original (se existir)
                    elif hasattr(sistema_integrado, 'executar_sistema_original'):
                        try:
                            # Inicia em thread separada para não bloquear interface
                            import threading
                            def run_sistema():
                                sistema_integrado.executar_sistema_original()
                            
                            thread = threading.Thread(target=run_sistema, daemon=True)
                            thread.start()
                            started = True
                            st.success("✅ Análise iniciada em background!")
                        except Exception as e:
                            st.error(f"❌ Erro ao iniciar análise: {str(e)}")
                            started = False
                    
                    # Opção 3: Simplesmente marca como running se não houver método específico
                    elif hasattr(sistema_integrado, 'running'):
                        sistema_integrado.running = True
                        started = True
                        st.success("✅ Sistema marcado como ativo!")
                    
                    else:
                        st.error("❌ Métodos de inicialização não encontrados no SistemaIntegrado")
                        started = False
                    ...
                    st.session_state["last_analysis_start"] = {
                        ...
                        "config_usado": config_temp,
                        "metodos_disponiveis": [attr for attr in dir(sistema_integrado) if not attr.startswith('_')],
                    ...
                    if started:
                        ...
                        st.warning("A análise não pôde ser iniciada. Verifique logs do backend.")
    ...
    with col_status:
        # ETAPA 5: Status detalhado do sistema
        if not mt5_ok:
            st.markdown("""
    ...
    # ETAPA 5: Debug avançado do sistema (opcional)
    if st.sidebar.checkbox("🔍 Debug Sistema Integrado", value=False):
        st.sidebar.markdown("**Status Detalhado:**")
        st.sidebar.write(f"- MT5: {'✅ Conectado' if mt5_ok else '❌ Desconectado'}")
        st.sidebar.write(f"- Backend: {'✅ Disponível' if backend_ok else '❌ Indisponível'}")
        st.sidebar.write(f"- Análise: {'🔄 Rodando' if analysis_running else '⏹️ Parada'}")
        
        if backend_ok:
            st.sidebar.write("**Detalhes do Backend:**")
            st.sidebar.write(f"- Running: {getattr(sistema_integrado, 'running', 'N/A')}")
            st.sidebar.write(f"- Analysis Thread: {getattr(sistema_integrado, 'analysis_thread', 'N/A')}")
            st.sidebar.write(f"- Status: {getattr(sistema_integrado, 'sistema_integrado_status', 'N/A')}")
        
        # Mostra último resultado do start_analysis_thread
        if "last_analysis_start" in st.session_state:
            st.sidebar.write("**Último Iniciar Análise:**")
            last_start = st.session_state["last_analysis_start"]
            st.sidebar.write(f"- Sucesso: {last_start.get('started', 'N/A')}")
            st.sidebar.write(f"- Timestamp: {last_start.get('timestamp', 'N/A')}")
            st.sidebar.write(f"- Tabela Disponível: {last_start.get('tabela_disponivel', 'N/A')}")
        
        # Mostra último erro, se houver
        if "last_analysis_error" in st.session_state:
            st.sidebar.write("**Último Erro:**")
            last_error = st.session_state["last_analysis_error"]
            st.sidebar.write(f"- Erro: {last_error.get('error', 'N/A')}")
            st.sidebar.write(f"- Timestamp: {last_error.get('timestamp', 'N/A')}")
    ...
    timeframe = st.sidebar.selectbox(
        ...
        index=["1 min", "5 min", "15 min", "30 min", "1 hora", "4 horas", "1 dia"].index(get_parametro_dinamico('timeframe', '1 dia'))
    )
    ...
    usar_multiplos_periodos = st.sidebar.radio(
        ...
        index=0 if not get_parametro_dinamico('usar_multiplos_periodos', True) else 1,
    ...
    if usar_multiplos_periodos == "Período Único":
        periodo_analise = st.sidebar.slider(
            "Período de Análise", 
            50, 250, int(get_parametro_dinamico('periodo_analise', 120)),  # Valor padrão centralizado
    ...
    zscore_threshold = st.sidebar.slider(
        "Limiar Z-Score", 
        0.5, 3.0, float(get_parametro_dinamico('zscore_min', 2.0)), 0.1,
        help="Limiar mínimo para considerar sinais"
    )

    max_posicoes = st.sidebar.slider("Máx. Posições Simultâneas", 1, 20, int(get_parametro_dinamico('max_posicoes', 6)))
    # Filtros
    ...
    r2_min = st.sidebar.slider("R² Mínimo", 0.1, 0.9, float(get_parametro_dinamico('r2_min', 0.5)), 0.05)
    beta_max = st.sidebar.slider("Beta Máximo", 0.1, 3.0, float(get_parametro_dinamico('beta_max', 2.0)), 0.05, help="Valor máximo absoluto de beta para aceitar o par")
    coint_pvalue_max = st.sidebar.slider("Cointegração Máxima", 0.01, 0.2, float(get_parametro_dinamico('coint_pvalue_max', 0.05)), 0.01, help="Valor máximo de p-valor para considerar o par cointegrado")


    # Dropdown para exibir ou ocultar campos de valores e limites operacionais (default: Oculto)
    exibir_valores = st.sidebar.selectbox(
        "Exibir valores e limites?",
        ["Exibir", "Ocultar"],
        index=1,  # 0 = Exibir, 1 = Ocultar (default)
        help="Permite mostrar ou esconder os campos de valores e limites operacionais no painel."
    )

    if exibir_valores == "Exibir":
        valor_operacao = st.sidebar.number_input("Valor máximo por operação (DEP)", min_value=1000, max_value=100000, value=int(get_parametro_dinamico('valor_operacao', calc_mod.valor_operacao)), step=500)
        valor_operacao_ind = st.sidebar.number_input("Valor máximo por operação (IND)", min_value=1000, max_value=100000, value=int(get_parametro_dinamico('valor_operacao_ind', calc_mod.valor_operacao_ind)), step=500)
        limite_lucro = st.sidebar.number_input("Limite de lucro por operação", min_value=10, max_value=1000, value=int(get_parametro_dinamico('limite_lucro', calc_mod.limite_lucro)), step=10)
        limite_prejuizo = st.sidebar.number_input("Limite de prejuízo por operação", min_value=10, max_value=1000, value=int(get_parametro_dinamico('limite_prejuizo', calc_mod.limite_prejuizo)), step=10)
    else:
        valor_operacao = calc_mod.valor_operacao
        valor_operacao_ind = calc_mod.valor_operacao_ind
        limite_lucro = calc_mod.limite_lucro
        limite_prejuizo = calc_mod.limite_prejuizo


    # Dropdown para exibir ou ocultar campos de horários operacionais (default: Oculto)
    exibir_horarios = st.sidebar.selectbox(
        "Exibir horários operacionais?",
        ["Exibir", "Ocultar"],
        index=1,  # 0 = Exibir, 1 = Ocultar (default)
        help="Permite mostrar ou esconder os campos de horários operacionais no painel."
    )

    if exibir_horarios == "Exibir":
        inicia_pregao = st.sidebar.number_input("Início do Pregão (h)", min_value=0, max_value=23, value=int(get_parametro_dinamico('inicia_pregao', calc_mod.inicia_pregao)))
        finaliza_pregao = st.sidebar.number_input("Final do Pregão (h)", min_value=0, max_value=23, value=int(get_parametro_dinamico('finaliza_pregao', calc_mod.finaliza_pregao)))
        finaliza_ordens = st.sidebar.number_input("Finaliza Novas Ordens (h)", min_value=0, max_value=23, value=int(get_parametro_dinamico('finaliza_ordens', calc_mod.finaliza_ordens)))
        ajusta_ordens = st.sidebar.number_input("Ajuste de Ordens (h)", min_value=0, max_value=23, value=int(get_parametro_dinamico('ajusta_ordens', calc_mod.ajusta_ordens)))
        horario_ajuste_stops = st.sidebar.number_input("Horário ajuste stops (h)", min_value=0, max_value=23, value=int(get_parametro_dinamico('horario_ajuste_stops', calc_mod.horario_ajuste_stops)))
        ajusta_ordens_minuto = st.sidebar.number_input("Minuto ajuste ordens", min_value=0, max_value=59, value=int(get_parametro_dinamico('ajusta_ordens_minuto', calc_mod.ajusta_ordens_minuto)))
        horario_remove_pendentes = st.sidebar.number_input("Horário remover pendentes (h)", min_value=0, max_value=23, value=int(get_parametro_dinamico('horario_remove_pendentes', calc_mod.horario_remove_pendentes)))
        horario_fechamento_total = st.sidebar.number_input("Horário fechamento total (h)", min_value=0, max_value=23, value=int(get_parametro_dinamico('horario_fechamento_total', calc_mod.horario_fechamento_total)))
    else:
        # Mantém os valores atuais das variáveis para não perder alterações
        inicia_pregao = calc_mod.inicia_pregao
        finaliza_pregao = calc_mod.finaliza_pregao
        finaliza_ordens = calc_mod.finaliza_ordens
        ajusta_ordens = calc_mod.ajusta_ordens
        horario_ajuste_stops = calc_mod.horario_ajuste_stops
        ajusta_ordens_minuto = calc_mod.ajusta_ordens_minuto
        horario_remove_pendentes = calc_mod.horario_remove_pendentes
        horario_fechamento_total = calc_mod.horario_fechamento_total
    ...
    config_final = {
        ...
        'intervalo_execucao': 60,
        # Parâmetros de horários operacionais (inclusão obrigatória para integração)
        'horario_ajuste_stops': horario_ajuste_stops,
        'horario_remove_pendentes': horario_remove_pendentes,
        'horario_fechamento_total': horario_fechamento_total,
        'ajusta_ordens_minuto': ajusta_ordens_minuto,
    ...
    if st.sidebar.button("💾 Salvar Perfil"):
        # Salva o perfil em config_perfil.json na mesma pasta do script
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile("w", delete=False, dir=os.path.dirname(config_path), encoding="utf-8") as tf:
                json.dump(config_final, tf, ensure_ascii=False, indent=2)
                tempname = tf.name
            os.replace(tempname, config_path)
            st.sidebar.success(f"Perfil salvo em {config_path}!")
            # Atualiza parâmetros do sistema integrado em tempo real, se rodando
            sistema_integrado = getattr(st.session_state.trading_system, 'sistema_integrado', None)
            if sistema_integrado and hasattr(sistema_integrado, 'atualizar_parametros'):
                try:
                    sistema_integrado.atualizar_parametros(config_final)
                    st.sidebar.info("Parâmetros do sistema integrados atualizados em tempo real!")
                except Exception as e:
                    st.sidebar.warning("Os novos parâmetros NÃO foram aplicados ao sistema integrado em tempo real. O sistema seguirá com os parâmetros anteriores.\nMotivo: " + str(e))
            else:
                st.sidebar.warning("Os novos parâmetros NÃO foram aplicados ao sistema integrado em tempo real. O sistema seguirá com os parâmetros anteriores, pois o backend não está disponível ou não suporta atualização dinâmica.")
        except Exception as e:
            st.sidebar.error(f"Erro ao salvar perfil: {str(e)}")

    # Botão de recarregar parâmetros do perfil (após Salvar Perfil)
    if st.sidebar.button("🔄 Recarregar Parâmetros do Perfil", key="btn_recarregar_perfil"):
        # Recarrega config_perfil.json e aplica em tempo real
        try:
            # Recarrega parâmetros globais do módulo de cálculo
            if hasattr(calc_mod, 'recarregar_parametros_perfil'):
                calc_mod.recarregar_parametros_perfil()
            # Atualiza parâmetros do sistema integrado (backend)
            sistema_integrado = getattr(st.session_state.trading_system, 'sistema_integrado', None)
            if sistema_integrado and hasattr(sistema_integrado, 'aplicar_parametros_dinamicos'):
                sistema_integrado.aplicar_parametros_dinamicos()
                st.sidebar.success("Parâmetros recarregados do perfil e aplicados ao sistema!")
            else:
                st.sidebar.warning("Parâmetros recarregados do perfil, mas backend não disponível para atualização dinâmica.")
        except Exception as e:
            st.sidebar.error(f"Erro ao recarregar parâmetros: {str(e)}")
    ...
    # Ao iniciar, tenta carregar config_perfil.json para usar como padrão
    # (isso deve ser feito no início do render_sidebar, mas para garantir, faz aqui também)
    config_path = os.path.join(os.path.dirname(__file__), "config_perfil.json")
    if os.path.exists(config_path):
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                ...
                config_loaded = json.loads(conteudo)
            # Atualiza todos os parâmetros do dashboard a partir do JSON
            ativos_selecionados = config_loaded.get('ativos_selecionados', ativos_selecionados)
            timeframe = config_loaded.get('timeframe', timeframe)
            periodo_analise = config_loaded.get('periodo_analise', periodo_analise)
            usar_multiplos_periodos = "Múltiplos Períodos" if config_loaded.get('usar_multiplos_periodos', usar_multiplos_periodos == "Múltiplos Períodos") else "Período Único"
            zscore_threshold = config_loaded.get('zscore_min', zscore_threshold)
            max_posicoes = config_loaded.get('max_posicoes', max_posicoes)
            filtro_cointegração = config_loaded.get('filtro_cointegracao', filtro_cointegração)
            filtro_r2 = config_loaded.get('filtro_r2', filtro_r2)
            filtro_beta = config_loaded.get('filtro_beta', filtro_beta)
            filtro_zscore = config_loaded.get('filtro_zscore', filtro_zscore)
            r2_min = config_loaded.get('r2_min', r2_min)
            beta_max = config_loaded.get('beta_max', beta_max)
            coint_pvalue_max = config_loaded.get('coint_pvalue_max', coint_pvalue_max)
            valor_operacao = config_loaded.get('valor_operacao', valor_operacao)
            valor_operacao_ind = config_loaded.get('valor_operacao_ind', valor_operacao_ind)
            limite_lucro = config_loaded.get('limite_lucro', limite_lucro)
            limite_prejuizo = config_loaded.get('limite_prejuizo', limite_prejuizo)
            horario_ajuste_stops = config_loaded.get('horario_ajuste_stops', horario_ajuste_stops)
            horario_remove_pendentes = config_loaded.get('horario_remove_pendentes', horario_remove_pendentes)
            horario_fechamento_total = config_loaded.get('horario_fechamento_total', horario_fechamento_total)
            ajusta_ordens_minuto = config_loaded.get('ajusta_ordens_minuto', ajusta_ordens_minuto)
            inicia_pregao = config_loaded.get('inicia_pregao', inicia_pregao)
            finaliza_pregao = config_loaded.get('finaliza_pregao', finaliza_pregao)
            finaliza_ordens = config_loaded.get('finaliza_ordens', finaliza_ordens)
            ajusta_ordens = config_loaded.get('ajusta_ordens', ajusta_ordens)
            # Se quiser restaurar outros parâmetros, adicione aqui
        except Exception as e:
            st.sidebar.warning(f"Falha ao carregar perfil salvo: {str(e)}. O sistema seguirá com os parâmetros atuais.")
    ...
    # REMOVIDO: Execução automática da análise para evitar recarregamento infinito
    # A análise só é iniciada/parada pelos botões de controle de análise.
    return config_final
...
def render_signals_table():
    ...
    # ========================================================================
    # Exibição explícita conforme exemplo do usuário
    if sistema.tabela_linha_operacao is not None and not sistema.tabela_linha_operacao.empty:
        st.markdown("### Sinais e Posições (Primeira Seleção)")
        st.dataframe(sistema.tabela_linha_operacao, use_container_width=True, hide_index=True, height=400)
    else:
        st.warning("Nenhum sinal disponível na primeira seleção.")
    return
...
def render_segunda_selecao():
    ...
    try:
        ...
        # Exibição explícita conforme exemplo do usuário
        if sistema.tabela_linha_operacao01 is not None and not sistema.tabela_linha_operacao01.empty:
            st.markdown("### Pares Validados (Segunda Seleção)")
            st.dataframe(sistema.tabela_linha_operacao01, use_container_width=True, hide_index=True, height=400)
        else:
            st.warning("Nenhum par validado disponível.")
        return
            
    # Métricas resumidas
    # (Se necessário, ajuste a lógica para garantir que df_segunda está definido e válido antes de usar este bloco)
    #col1, col2, col3, col4 = st.columns(4)
    #with col1:
    #    total_pl = 0
    ...
    # Tabela principal: exibe todas as colunas da tabela_linha_operacao01
    #st.markdown("---")
    #st.dataframe(df_segunda, use_container_width=True, hide_index=True, height=400)
    #else:
