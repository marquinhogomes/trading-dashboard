...
def get_mt5_data(symbol='WIN$', n_bars=90000, timeframe=mt5.TIMEFRAME_M1):
...
class IBOVPredictor:
    ...
    def _get_session_factor(self, hour, minute):
        ...
        opening_mask = ((hour >= 9) & (hour < 10)) #(hour == 10)
        time_factor[opening_mask] = 1.2
        
        # Fechamento (16:00-18:00) - Maior volatilidade
        closing_mask = ((hour >= 10) & (hour < 11)) #(hour >= 15)
        time_factor[closing_mask] = 1.2
        
        # Meio do dia (12:00-14:00) - Menor volatilidade
        midday_mask = ((hour >= 11) & (hour < 18))
        time_factor[midday_mask] = 0.8
        
        return time_factor

    def create_target(self, df, threshold=0.0003): #0.0003
        """
    ...
    def train_models(self, X, y):
        ...
        # scale_pos_weight e use_label_encoder não são usados em multiclasse, então removemos
...
def main():
    ...
    df = get_mt5_data(symbol='WIN$', n_bars=90000)
